...
#  - NEW: Riktkurser 1–3 år = "bästa scenario" – MoS per bucket (A 5%, B 8%, C 12%)
...
def _best_case_row(methods_df: pd.DataFrame) -> Dict[str, Any]:
    """
    'Bästa scenario' = max-pris över alla metoder (exkl. fair_value) per horisont.
    """
    cols = ["Idag", "1 år", "2 år", "3 år"]
    ...
    sub = methods_df[methods_df["Metod"] != "fair_value"].copy()
    for c in cols:
        try:
...
def compute_methods_for_row(row: pd.Series, settings: Dict[str, str] | None = None, fx_map: Dict[str, float] | None = None) -> Dict[str, Any]:
    ...
    # --- Fair Value (familjemedian) = IDAG ---
    fv_row = _compute_fair_value_row_v2(methods_df, price)
    # --- Bästa scenario (max per horisont) ---
    best_row = _best_case_row(methods_df)
...
# ============================================================
# app.py — Aktieanalys & investeringsförslag
# Del 4/6: Vyer — Analys & Redigering (UI)
#
#  - render_analysis_view(): välj bolag → räkna riktkurser → visa metoder
#  - render_methods_table(): snygg tabell av methods_df
#  - render_full_db_table(): enkel vy längst ner med hela databasen (alla kolumner)
#  - Små hjälpare för formatering och upp-/nedsida
#
#  OBS:
#  • All logik för själva beräkningarna ligger i Del 3 (compute_methods_for_row).
#  • Lagring/Sheets-uppdateringar/Batch körs i Del 5–6 (oförändrat jämfört med basversionen).
# ============================================================

# -------------------------
# Hjälpare (format & nyckeltal)
# -------------------------
def _fmt_price(v: Any, currency: str | None = None) -> str:
    x = _f(v)
    if x is None:
        return "—"
...
def _upside(target: Optional[float], price: Optional[float]) -> Optional[float]:
    if _pos(target) is None or _pos(price) is None or float(price) == 0:
        return None
    return (float(target) / float(price)) - 1.0
...
def render_methods_table(methods_df: "pd.DataFrame", currency: str) -> None:
    ...
    for col in ["Idag", "1 år", "2 år", "3 år"]:
        if col in df.columns:
            df[col] = df[col].apply(lambda v: _fmt_price(v, currency))
...
# -------------------------
def render_full_db_table(df: "pd.DataFrame") -> None:
    st.markdown("---")
    st.markdown("### Hela databasen (enkel tabell)")
    if df is None or (hasattr(df, "empty") and df.empty):
        st.info("Databasen är tom.")
        return
    # Visa alla kolumner utan redigering/sortering (Streamlit sorterar inte automatiskt i dataframe)
    st.dataframe(df, use_container_width=True)

# -------------------------
# Analysvy
# -------------------------
def render_analysis_view(df_data: "pd.DataFrame", settings: Dict[str, Any] | None = None, fx_map: Dict[str, float] | None = None) -> None:
    """
    UI-flöde:
      1) Välj bolag (ticker) i rullista.
      2) Beräkna fair value + riktkurser via compute_methods_for_row.
      3) Visa kort: pris, FV idag, 1/2/3 år, upp-/nedsida.
      4) Visa metodtabell.
      5) Visa hela databasen längst ner (krav i basversionen).
    """
    settings = settings or get_settings_map()
    ...
    if df_data is None or (hasattr(df_data, "empty") and df_data.empty):
        st.warning("Ingen data att analysera. Lägg till bolag i vyn 'Lägg till/uppdatera' först.")
        return

    tickers = df_data["Ticker"].dropna().astype(str).sort_values().unique().tolist()
    preselect = st.session_state.get("analysis_selected_ticker") or (tickers[0] if tickers else "")
    picked = st.selectbox("Välj bolag", tickers, index=_safe_int(tickers.index(preselect)) if preselect in tickers else 0, key="analysis_selected_ticker")

    # Filtrera fram vald rad (första match om flera)
    row = df_data[df_data["Ticker"].astype(str) == str(picked)].iloc[0] if not df_data.empty else None
    if row is None:
        st.info("Kunde inte hitta vald ticker i tabellen.")
        render_full_db_table(df_data)
        return

    with st.spinner("Beräknar riktkurser…"):
        payload = compute_methods_for_row(row, settings, fx_map)

    currency = (payload.get("currency") or str(row.get("Valuta") or "USD")).upper()
    price    = _f(payload.get("price"))
    fv0      = _f(payload.get("target_today"))
    fv1      = _f(payload.get("target_1y"))
    fv2      = _f(payload.get("target_2y"))
    fv3      = _f(payload.get("target_3y"))

    # -------------------------
    # Översiktskort (price + FV)
    # -------------------------
    colA, colB, colC, colD, colE = st.columns(5)
    with colA:
        st.metric("Aktuell kurs", _fmt_price(price, currency))
    with colB:
        st.metric("Fair value (idag)", _fmt_price(fv0, currency), delta=_fmt_pct(_upside(fv0, price)))
    with colC:
        st.metric("Riktkurs 1 år (MoS)", _fmt_price(fv1, currency), delta=_fmt_pct(_upside(fv1, price)))
    with colD:
        st.metric("Riktkurs 2 år (MoS)", _fmt_price(fv2, currency), delta=_fmt_pct(_upside(fv2, price)))
    with colE:
        st.metric("Riktkurs 3 år (MoS)", _fmt_price(fv3, currency), delta=_fmt_pct(_upside(fv3, price)))
    ...
    # Metodtabell
    # -------------------------
    render_methods_table(payload.get("methods_df"), currency)
...
# -------------------------
# (Valfri) Redigeringskort för vald rad
#  – OBS: Själva sparandet mot Sheets görs i Del 5–6 (oförändrat).
# -------------------------
def render_editor_card_for_row(row: "pd.Series") -> Dict[str, Any]:
    """
    Lättvikts-editor som returnerar ett dict med ev. uppdaterade värden.
    Denna funktion sparar inte – Del 5–6 ansvarar för persistens.
    """
    st.markdown("#### Redigera fält (snabb)")

    def _ti(label: str, key: str, default: Any = ""):
        return st.text_input(label, value=str(default or ""), key=key)

    # Minimalt urval; hela editor-vyn i basen finns kvar i Del 5–6
    new_price = _ti("Aktuell kurs (manuell)", f"ed_price_{row.name}", row.get("Aktuell kurs"))
    new_rev   = _ti("Rev TTM", f"ed_rev_{row.name}", row.get("Rev TTM"))
    new_eps   = _ti("EPS TTM", f"ed_eps_{row.name}", row.get("EPS TTM"))
    new_ev_s  = _ti("EV/Revenue", f"ed_evs_{row.name}", row.get("EV/Revenue"))
    new_ev_e  = _ti("EV/EBITDA", f"ed_eve_{row.name}", row.get("EV/EBITDA"))
    new_pef   = _ti("PE FWD", f"ed_pef_{row.name}", row.get("PE FWD"))

    return {
        "Aktuell kurs": _f(new_price),
        "Rev TTM": _f(new_rev),
        "EPS TTM": _f(new_eps),
        "EV/Revenue": _f(new_ev_s),
        "EV/EBITDA": _f(new_ev_e),
        "PE FWD": _f(new_pef),
    }
...
# ============================================================
# app.py — Aktieanalys & investeringsförslag
# Del 6/6: Ranking (Fair Value v2), Hem & Main()
#
#  - compute_methods_for_row() enligt basversion 2025-11-16
#  - 🏆 Ranking: räknar riktkurser (Idag/1/2/3 år) + Bull/Bear 1 år
#  - 🏠 Hem: snabböversikt + hela Data-tabellen
#  - Main: sidopanel, sidrouting, bootstrapping av cache/session
# ============================================================

# =========================
# Fair Value — beräkningsmotor
# =========================
def _get_setting_float(settings: Dict[str, Any], key: str, default: float) -> float:
    try:
...
def _decayed_multiple(m0: Optional[float], decay: float, years: int) -> Optional[float]:
    """
    Multipel-decay per år med golv 60% av startmultipeln.
    m_t = max(m0*(1-decay)^t, m0*0.60)
    """
    if m0 is None or not math.isfinite(m0) or m0 <= 0:
        return None
    try:
        mt = m0 * ((1.0 - decay) ** years)
        mt = max(mt, m0 * 0.60)
        return mt
    except Exception:
        return None
...
def _median_ignore_nones(values: list[Optional[float]]) -> Optional[float]:
...
def compute_methods_for_row(row: pd.Series, settings: Dict[str, Any]) -> Dict[str, Any]:
    """
    Implementerar Fair Value v2 (bas 2025-11-16):
    • Inputs: Yahoo-snapshot + EPS/Rev-estimat ur Data-bladet
    • Clamp: Rev CAGR [-10%, +35%], EPS CAGR [-20%, +35%]
    • PE-ankare = vikt mellan PE TTM & PE FWD (default 50/50)
    • Multipel-decay per år (default 10%), golv 60% av startmultipel
    • Metoder: pe_hist_vs_eps, ev_sales, ev_ebitda, p_b
    • Fair Value = median över metodfamiljer (pe, ev_s, ev_e, pb)
      (I “Idag” filtreras värden inom ±0.5% av aktuell kurs; om allt försvinner → fall-back till pe-raden)
    """
    # --- Hämta inställningar ---
    w_ttm = _get_setting_float(settings, "pe_anchor_weight_ttm", 0.50)
    w_ttm = min(1.0, max(0.0, w_ttm))
    decay = _get_setting_float(settings, "multiple_decay", 0.10)
    decay = min(0.50, max(0.0, decay))  # sanity

    # --- Plocka datapunkter (tål None) ---
    price        = _f(row.get("Aktuell kurs"))
    shares_out   = _f(row.get("Utestående aktier"))
    net_debt     = _f(row.get("Net debt"))
    rev_ttm      = _f(row.get("Rev TTM"))
    ebitda_ttm   = _f(row.get("EBITDA TTM"))
    eps_ttm      = _f(row.get("EPS TTM"))
    pe_ttm       = _f(row.get("PE TTM"))
    pe_fwd       = _f(row.get("PE FWD"))
    ev_rev       = _f(row.get("EV/Revenue"))
    ev_ebitda    = _f(row.get("EV/EBITDA"))
    p_b          = _f(row.get("P/B"))
    bvps         = _f(row.get("BVPS"))
    rev_cagr_h   = _f(row.get("Rev CAGR"))
    eps_cagr_h   = _f(row.get("EPS CAGR"))
    eps_cagr_lt  = _f(row.get("EPS CAGR LT"))  # valfri kolumn; används om den finns

    # Härleder EPS TTM om saknas men PE & price finns
    if eps_ttm is None and price is not None and pe_ttm is not None and pe_ttm > 0:
        try:
    ...
    # Clampade tillväxter
    g_rev = _clamp(rev_cagr_h, -0.10, 0.35)
    if g_rev is None:
        g_rev = 0.0

    # EPS-g villkor: hist → LT → rev → 0
    g_eps = _clamp(eps_cagr_h, -0.20, 0.35)
    if g_eps is None:
        g_eps = _clamp(eps_cagr_lt, -0.20, 0.35)
    if g_eps is None:
        g_eps = _clamp(rev_cagr_h, -0.20, 0.35)
    if g_eps is None:
        g_eps = 0.0

    # PE-ankare (fallback om båda saknas → 15)
    if pe_ttm is None and pe_fwd is None:
        ...
        pe_anchor0 = (w_ttm * pe_ttm) + ((1.0 - w_ttm) * pe_fwd)

    # Multipelbanor (0,1,2,3 år)
    pe_mults      = [ _decayed_multiple(pe_anchor0, decay, t)      for t in (0,1,2,3) ]
    evs_mults     = [ _decayed_multiple(ev_rev,     decay, t)      for t in (0,1,2,3) ]
    ev_ebitda_ms  = [ _decayed_multiple(ev_ebitda,  decay, t)      for t in (0,1,2,3) ]
    pb_mults      = [ _decayed_multiple(p_b,        decay, t)      for t in (0,1,2,3) ]

    # Intäktsbana
    rev_path = [None, None, None, None]
    if rev_ttm is not None and rev_ttm > 0:
        rev_path[0] = float(rev_ttm)
        for i in (1,2,3):
            rev_path[i] = rev_path[i-1] * (1.0 + g_rev)

    # EBITDA-bana (skalar TTM-marginal)
    ebitda_path = [None, None, None, None]
    if ebitda_ttm is not None and ebitda_ttm > 0 and rev_ttm and rev_ttm > 0 and rev_path[0] is not None:
        try:
            margin0 = float(ebitda_ttm) / float(rev_ttm)
            for i in (0,1,2,3):
                if rev_path[i] is not None:
                    ebitda_path[i] = rev_path[i] * margin0
        except Exception:
            pass

    # EPS-bana
    eps_path = [None, None, None, None]
    if eps_ttm is not None and math.isfinite(eps_ttm):
        eps_path[0] = float(eps_ttm)
        for i in (1,2,3):
            eps_path[i] = eps_path[i-1] * (1.0 + g_eps)

    # Hjälpare för equity-pris från EV
    def _equity_price_from_ev(ev_val: Optional[float]) -> Optional[float]:
        if ev_val is None:
            return None
        if shares_out is None or not math.isfinite(shares_out) or shares_out <= 0:
            return None
        nd = float(net_debt) if (net_debt is not None and math.isfinite(net_debt)) else 0.0
        eq = ev_val - nd
        if eq <= 0:
            return 0.0
        return eq / float(shares_out)

    # Beräkna metoder per horisont
    pe_prices   = [None, None, None, None]
    evs_prices  = [None, None, None, None]
    eve_prices  = [None, None, None, None]
    pb_prices   = [None, None, None, None]

    for i in (0,1,2,3):
        # PE * EPS
        if eps_path[i] is not None and pe_mults[i] is not None:
            pe_prices[i] = max(0.0, eps_path[i] * pe_mults[i])

        # EV/Sales
        if rev_path[i] is not None and evs_mults[i] is not None:
            ev_val = rev_path[i] * evs_mults[i]
            evs_prices[i] = _equity_price_from_ev(ev_val)

        # EV/EBITDA
        if ebitda_path[i] is not None and ev_ebitda_ms[i] is not None:
            ev_val = ebitda_path[i] * ev_ebitda_ms[i]
            eve_prices[i] = _equity_price_from_ev(ev_val)

        # P/B
        if pb_mults[i] is not None and bvps is not None and math.isfinite(bvps) and bvps > 0:
            pb_prices[i] = max(0.0, pb_mults[i] * float(bvps))

    # Fair value per horisont = median över familjer
    def _fv_for(i: int, filter_vs_price_today: bool) -> Optional[float]:
        families = {
            "pe":   pe_prices[i],
            "ev_s": evs_prices[i],
            "ev_e": eve_prices[i],
        ...
        if filter_vs_price_today and price is not None and math.isfinite(price) and price > 0:
            pr = float(price)
            pr_lo, pr_hi = pr * 0.995, pr * 1.005
            vals = [v for v in vals if (v is not None and (v < pr_lo or v > pr_hi))]

        fv = _median_ignore_nones(vals)
        if fv is None and filter_vs_price_today:
            # fall-back: ta PE-värdet om allt filtrerats bort
            fv = pe_prices[i]
        return fv

    fv_today = _fv_for(0, True)
    fv_1y    = _fv_for(1, False)
    fv_2y    = _fv_for(2, False)
    fv_3y    = _fv_for(3, False)

    # Bull/Bear 1 år (enkelt band = max/min av familjvärden på 1 år)
    fam_1y = [v for v in (pe_prices[1], evs_prices[1], eve_prices[1], pb_prices[1]) if v is not None]
    bull_1 = max(fam_1y) if fam_1y else None
    bear_1 = min(fam_1y) if fam_1y else None
    ...
    meta = {
        ...
        "eps_cagr_used": g_eps,
        "paths": {
            "rev": rev_path, "ebitda": ebitda_path, "eps": eps_path
        },
        "families": {
            "pe": pe_prices, "ev_s": evs_prices, "ev_e": eve_prices, "pb": pb_prices
        }
    }

    out = {
        "today": None if fv_today is None else round(float(fv_today), 2),
        "y1":    None if fv_1y    is None else round(float(fv_1y), 2),
        "y2":    None if fv_2y    is None else round(float(fv_2y), 2),
        "y3":    None if fv_3y    is None else round(float(fv_3y), 2),
        "bull1": None if bull_1   is None else round(float(bull_1), 2),
        "bear1": None if bear_1   is None else round(float(bear_1), 2),
        "pe_anchor": None if pe_anchor0 is None else round(float(pe_anchor0), 2),
...
# =========================
def page_ranking():
    st.header("🏆 Ranking (Fair Value v2)")
    df = st.session_state.get("DATA")
    if df is None or (isinstance(df, pd.DataFrame) and df.empty):
        df = read_data_df()
    if df is None or df.empty:
    ...
    with c1:
        do_write = st.checkbox("Skriv tillbaka till Data", value=True)
    with c2:
        show_preview = st.checkbox("Visa förhandsvisning", value=True)
    with c3:
        st.caption("Fält som uppdateras: Riktkurs idag/1/2/3 år, Bull 1 år, Bear 1 år, Senast rankad, PE-ankare, Multipel-decay.")

    if st.button("🚀 Kör ranking"):
        target = tickers if len(sel) == 0 else sel
        base = df.copy()
        updated_rows = []
        ...
        for i, tkr in enumerate(target, start=1):
            try:
                ...
                mask = base["Ticker"].astype(str).str.upper() == str(tkr).upper()
                if not mask.any():
                    continue
                idx = base.index[mask][0]
                row = base.loc[idx]

                res = compute_methods_for_row(row, settings)
                # Skriv resultat in i Data
                base.at[idx, "Riktkurs idag"]  = res["today"]
                base.at[idx, "Riktkurs 1 år"]  = res["y1"]
                base.at[idx, "Riktkurs 2 år"]  = res["y2"]
                base.at[idx, "Riktkurs 3 år"]  = res["y3"]
                base.at[idx, "Bull 1 år"]      = res["bull1"]
                base.at[idx, "Bear 1 år"]      = res["bear1"]
                base.at[idx, "Senast rankad"]  = now_stamp()
                base.at[idx, "PE-ankare"]      = res.get("pe_anchor")
                base.at[idx, "Decay multipel"] = res.get("decay")

                updated_rows.append({
                    "Ticker": tkr,
                    "Riktkurs idag": res["today"],
                    "Riktkurs 1 år": res["y1"],
                    "Riktkurs 2 år": res["y2"],
                    "Riktkurs 3 år": res["y3"],
                    "Bull 1 år": res["bull1"],
                    "Bear 1 år": res["bear1"],
                })
            except Exception as e:
        ...
        if do_write:
            try:
                ...
                st.success(f"Klar. {len(target)} bolag rankade och sparade.")
            except Exception as e:
                st.error(f"Kunde inte skriva till Google Sheets: {e}")
        else:
            st.info("Körning klar (inget sparat).")

        if show_preview and updated_rows:
            prev = pd.DataFrame(updated_rows)
            _show_df(prev, height=360, use_container_width=True)
...
def page_home():
    ...
    # Snabbsiffror
    try:
        pos = _position_value_tables(data_df, fx_map)
        tot_sek = float(pos["Värde (SEK)"].sum()) if not pos.empty else 0.0
        st.metric("Totalt portföljvärde (SEK)", f"{tot_sek:,.0f}".replace(",", " "))
    except Exception:
        pass

    st.markdown("#### Hela Data-tabellen (read-only)")
    try:
        _show_df(data_df, height=420, use_container_width=True)
    except Exception:
        st.table(data_df.head(200))
...
def main():
    ...
    choice = st.sidebar.radio("Välj sida", list(PAGES.keys()), index=0)
    st.sidebar.markdown("---")
    st.sidebar.caption("Basversion 2025-11-16 • Fair Value v2")
