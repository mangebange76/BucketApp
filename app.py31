...
def yahoo_fetch_for_ticker(sym: str) -> Dict[str, Any]:
    ...
    out: Dict[str, Any] = {
        ...
        "EV/EBITDA": _yf_ev_ebitda_ttm(tkr),
        "P/B": _yf_p_b(tkr),
        "BVPS": _yf_bvps(tkr),
...
#  - Bull/Bear (1 år) skrivs när beräkningsmotorn tillhandahåller dem
# ============================================================
...
def _call_valuation_engine(row: pd.Series) -> Dict[str, Any]:
    """
    Abstraktionslager så vi inte bryter mot basversionen:
    - Om compute_methods_for_row finns, använd den (retur ska innehålla targets/bull/bear/metadatat).
    - Annars prova compute_strict_valuation / compute_strict_for_row.
    - Som sista utväg: enkel fallback som lämnar None (ingen skrivning).
    """
    try:
        if "compute_methods_for_row" in globals():
            return compute_methods_for_row(row)  # förväntas returnera dict
    except Exception:
        pass
    for fn in ("compute_strict_valuation", "compute_strict_for_row", "compute_valuations_for_row"):
        try:
            if fn in globals():
                return globals()[fn](row)  # type: ignore[index]
        except Exception:
            continue
    return {}

def _extract_targets(payload: Dict[str, Any]) -> Dict[str, Optional[float]]:
    # Tillåter flera nyckelnamn från olika motorer
    kmap = {
        "today": ["target_today","fv_today","riktkurs_idag","today_target"],
        "y1":    ["target_1y","fv_1y","riktkurs_1_ar","rk_1y"],
        "y2":    ["target_2y","fv_2y","riktkurs_2_ar","rk_2y"],
        "y3":    ["target_3y","fv_3y","riktkurs_3_ar","rk_3y"],
        "bull":  ["bull_1y","bull","bull_target"],
        "bear":  ["bear_1y","bear","bear_target"],
        "method":["method","Metod"],
        "input": ["input","Input-sammanfattning","inputs"],
        "note":  ["note","Kommentar","comment"],
    }
    out: Dict[str, Optional[float] | str] = {
        "today": None, "y1": None, "y2": None, "y3": None,
        "bull": None, "bear": None, "method": "", "input": "", "note": ""
    }
    for key, aliases in kmap.items():
        for a in aliases:
            if a in payload and payload[a] not in (None, ""):
                out[key] = payload[a]
                break
    # Normalisera till floats där det ska vara tal
    for numk in ("today","y1","y2","y3","bull","bear"):
        v = out[numk]
        if v is None or v == "":
            continue
        try:
            out[numk] = float(v)  # type: ignore[assignment]
        except Exception:
            out[numk] = None
    return out  # type: ignore[return-value]

# -------------------------
# 🏆 Ranking (beräkna riktkurser/fair value)
# -------------------------
def _rank_one_row(row: pd.Series) -> Dict[str, Any]:
    payload = _call_valuation_engine(row) or {}
    ext = _extract_targets(payload)
    return {
        "Riktkurs idag": ext["today"],
        "Riktkurs 1 år": ext["y1"],
        "Riktkurs 2 år": ext["y2"],
        "Riktkurs 3 år": ext["y3"],
        "Bull 1 år": ext["bull"],
        "Bear 1 år": ext["bear"],
        "Metod": ext.get("method") or payload.get("method") or payload.get("Metod"),
        "Input-sammanfattning": ext.get("input") or payload.get("input") or payload.get("Input-sammanfattning"),
        "Kommentar": ext.get("note") or payload.get("note") or payload.get("Kommentar"),
    }
...
def page_ranking():
    ...
    st.caption("Notera: riktkurser beräknas i aktiens egen valuta (ingen valutakonvertering).")

    if not calc_btn:
        st.dataframe(df[["Ticker","Bolagsnamn","Valuta","Riktkurs idag","Riktkurs 1 år","Riktkurs 2 år","Riktkurs 3 år"]],
    ...
    prog.empty()
    status.empty()
    st.success(f"Klar. {len(target)} tickers uppdaterade.")
    st.dataframe(out.loc[out["Ticker"].astype(str).str.upper().isin([t.upper() for t in target]),
                         ["Ticker","Valuta","Riktkurs idag","Riktkurs 1 år","Riktkurs 2 år","Riktkurs 3 år"]],
                 use_container_width=True, hide_index=True)
