...
def refresh_fx_live() -> None:
    ...
    # CHANGED: uppdatera session direkt så UI kan visa timestamp utan att vänta på cache-ttl
    try:
        st.session_state["FX_TS"] = ts
    except Exception:
        pass

    # CHANGED: invalidera även settings-cache så FX_LAST_UPDATE_TS syns direkt i UI
    try:
        get_settings_map.clear()
    except Exception:
        try:
            st.cache_data.clear()
        except Exception:
