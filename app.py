...
#  • 5-års historisk CAGR (Revenue & EPS) med clamp
#  • Wrapper-funktioner som Editor förväntar sig
#  • 🔁 Förbättrad utdelningslogik (annual, frekvens, nästa datum/belopp)
# ============================================================
...
def _parse_ts_to_date(x) -> Optional[dt.date]:
    """Stödjer epoch (sek), str YYYY-MM-DD, pandas/np tidsformat."""
    try:
        if x is None or (isinstance(x, float) and (np.isnan(x))):
            return None
        if isinstance(x, (int, float)) and not isinstance(x, bool):
            # epoch sekunder → UTC-datum
...
# -----------------------------
# 🔁 Förbättrad utdelningslogik
# -----------------------------
def _div_series(t: yf.Ticker) -> pd.Series:
    """Säker hämtning av utdelningsserie (per aktie), indexerad på datum."""
    try:
        s = t.dividends
...
def _median_days_gap(dates: pd.DatetimeIndex, lookback: int = 8) -> Optional[int]:
    if dates is None or len(dates) < 2:
        return None
    try:
        diffs = np.diff(dates.values.astype("datetime64[D]")).astype(int)
        if diffs.size == 0:
            return None
        tail = diffs[-lookback:] if diffs.size >= lookback else diffs
        return int(np.median(tail))
    except Exception:
        return None

def _infer_div_freq_from_history(s: pd.Series) -> tuple[Optional[str], Optional[int]]:
    """Returnerar (freq_code, n_per_year) från historiska intervall."""
    if s is None or s.empty:
        return None, None
    gap = _median_days_gap(s.index)
    if gap is None:
        # fallback via count över 400 dagar
        one_year_ago = pd.Timestamp.today().normalize() - pd.Timedelta(days=400)
        cnt = int(s[s.index >= one_year_ago].shape[0])
        if cnt >= 10:
            return "M", 12
        if 3 <= cnt <= 5:
            return "Q", 4
        ...
        if cnt == 1:
            return "A", 1
        return None, None
    # klassificera på median-intervall
    if gap <= 45:
        return "M", 12
    if gap <= 120:
        return "Q", 4
...
def _ttm_dividend_sum(s: pd.Series) -> Optional[float]:
    if s is None or s.empty:
        return None
    try:
...
def _infer_next_div_date_and_amount(info: dict, s: pd.Series, freq_code: Optional[str], n_per_year: Optional[int]) -> tuple[Optional[dt.date], Optional[float]]:
    """Bästa möjliga gissning av nästa betalningsdatum och belopp."""
    # Kända fält från Yahoo
    next_pay_date = _parse_ts_to_date(info.get("dividendDate"))  # ibland framtida
    ex_date = _parse_ts_to_date(info.get("exDividendDate"))      # kan vara kommande eller senaste
    last_paid_date = s.index.max().date() if (s is not None and not s.empty) else None
    last_paid_amt  = float(s.iloc[-1]) if (s is not None and not s.empty) else None

    # Om Yahoo tydligt ger framtida betalningsdatum – använd det.
    today = dt.date.today()
    if next_pay_date and next_pay_date >= today:
        return next_pay_date, last_paid_amt  # belopp okänt → använd senaste om inget annat

    # Annars: om ex-datum ligger i framtiden, lägg på typisk lagg
    lag_days_map = {"M": 15, "Q": 30, "S": 45, "A": 60}
    if ex_date and ex_date >= today:
        lag = lag_days_map.get(freq_code or "Q", 30)
        try:
            return (ex_date + dt.timedelta(days=lag)), last_paid_amt
        except Exception:
            pass

    # Annars: prognos = senaste betaldatum + medianintervall
    if last_paid_date and s is not None and not s.empty:
        gap = _median_days_gap(s.index) or (365 // max(n_per_year or 1, 1))
        try:
            return (last_paid_date + dt.timedelta(days=gap)), last_paid_amt
        except Exception:
            pass

    return None, None

def _compute_dividend_meta(t: yf.Ticker, info: dict) -> dict:
    """
    Returnerar:
      - dps_annual (prioritera forward dividendRate)
      - div_freq ('M','Q','S','A')
      - next_div_date (date)
      - next_div_ps (float)
    """
    s = _div_series(t)
    freq_code, n_per_year = _infer_div_freq_from_history(s)

    # Forward annual DPS (dividendRate) eller trailing (TTM) eller sista*freq
    forward_annual = _f(info.get("dividendRate"))
    trailing_ttm   = _ttm_dividend_sum(s)
    last_amt       = _safe_last(s)

    dps_annual = None
    # Prioritetsordning: forward > TTM > last*n
    for cand in (forward_annual, trailing_ttm, (last_amt * (n_per_year or 0) if last_amt and n_per_year else None)):
        if _f(cand) is not None:
            dps_annual = float(_f(cand))
            break

    # Nästa datum + belopp
    next_date, next_amt_from_history = _infer_next_div_date_and_amount(info, s, freq_code, n_per_year)

    # Nästa belopp – använd forward om möjligt
    next_ps = None
    if _pos(dps_annual) and n_per_year:
        next_ps = float(dps_annual) / float(n_per_year)
    elif _pos(next_amt_from_history):
        next_ps = float(next_amt_from_history)
    elif _pos(last_amt):
        next_ps = float(last_amt)

    return {
        "dps_annual": dps_annual,              # årlig takt (före källskatt)
        "div_freq": freq_code,                 # 'M','Q','S','A' eller None
        "next_div_date": next_date,            # betalningsdatum (bästa gissning)
        "next_div_ps": next_ps,                # nästa teoretiska utd/aktie
    }

# -----------------------------
# Snabb snapshot (pris, valuta, shares, net debt, multiplar, utdelning)
# -----------------------------
def _fast_snapshot(ticker: str) -> Dict[str, Optional[float]]:
    ...
    ev_rev, ev_ebitda, pb, bvps, dps_annual_last = None, None, None, None, None
    ...
    # Utdelning (senaste betalda värde om exponerat)
    try:
        dps_annual_last = _f(inf.get("lastDividendValue"))
    except Exception:
        pass
    ...
    return {
        ...
        "p_b": pb,
        "bvps": bvps,
        # OBS: detta är bara senaste betalningen; hel årlig takt beräknas i _compute_dividend_meta
        "dps_annual_last": dps_annual_last,
        "raw_info": inf,  # skicka vidare för utdelningslogik
    }
...
def fetch_from_yahoo(ticker: str) -> Dict[str, Any]:
    ...
    Hämtar ett robust snapshot från Yahoo:
      ...
      • 🆕 Utdelningar: årlig takt, frekvens, nästa datum & nästa belopp
    """
    t = _yf_ticker(ticker)
    ...
    divm = _compute_dividend_meta(t, snap.get("raw_info") or {})
    ...
    out = {
        ...
        # 🆕 Utdelning (förbättrad)
        "dps_annual": divm.get("dps_annual"),                 # årlig takt
        "div_freq": divm.get("div_freq"),                     # 'M','Q','S','A'
        "next_div_date": divm.get("next_div_date"),           # betalningsdatum (approx om behövs)
        "next_div_ps": divm.get("next_div_ps"),               # nästa utd/aktie
        "last_div_ps": snap.get("dps_annual_last"),           # senaste rapporterade betalning (per-aktie)
        # Historik (clampad)
...
def merge_yahoo_into_row(row: pd.Series, y: Dict[str, Any]) -> pd.Series:
    ...
    for col, key in mapping.items():
        val = y.get(key, None)
        # Datum skrivs som dt.date eller YYYY-MM-DD sträng; övrigt numeriskt/sträng
        if val is None:
            continue
        if isinstance(val, (int, float)) and not math.isfinite(val):
            continue
        # Konvertera datumobjekt till str för smidigare skrivning till Sheets
        if col == "Nästa utdelningsdatum":
            try:
...
def fetch_yahoo_snapshot(ticker: str) -> Dict[str, Any]:
    ...
    Returnerar ett "editor-vänligt" snapshot-dict med nycklar:
      ...
      annual_dividend, dividend_frequency, next_dividend_date, next_dividend_ps
    """
    t = _yf_ticker(ticker)
    ...
    return {
        ...
        "next_dividend_date": base.get("next_div_date"),
        "next_dividend_ps": base.get("next_div_ps"),
    }
...
def render_portfolio_view(data_df: pd.DataFrame, fx_map: dict[str, float]) -> None:
    ...
    if not pf.empty and "Bucket" in data_df.columns:
        # Koppla tillbaka Bucket från ursprungs-DF (pf innehåller redan Bucket)
        bucket_tbl = (
...
# ============================================================
# Del 6/6 — App-entry & router
#  • Start/init (läser DATA & FX till session)
#  • Sidomeny / navigering
#  • Felhantering runt vyerna
# ============================================================

def _ensure_session_loaded():
    """Ladda DATA och FX till session_state om saknas."""
    # DATA
...
def _maybe_auto_refresh_fx():
    """Uppdatera FX om Settings säger så och FX-kartan ser tom/åldrad ut."""
    try:
        s = get_settings_map()
        auto = str(s.get("auto_refresh_on_start", "0")) == "1"
        if auto:
            # Kör en snabb uppdatering och ladda om FX-kartan
            _load_fx_and_update_sheet()
            st.session_state["FX"] = get_fx_map()
    except Exception as e:
...
def _sidebar_header():
    st.sidebar.markdown("## ⚡️ Aktieanalys & investeringsförslag")
    df = st.session_state.get("DATA")
    if df is not None and not df.empty:
        try:
            owned = (pd.to_numeric(df.get("Antal aktier"), errors="coerce") > 0).sum()
            st.sidebar.caption(f"Rader: **{len(df)}** · Innehav >0: **{owned}**")
        except Exception:
            st.sidebar.caption(f"Rader: **{len(df)}**")

    st.sidebar.markdown("---")
    col1, col2 = st.sidebar.columns(2)
    with col1:
        if st.button("🔄 Läs DATA"):
            try:
                st.session_state["DATA"] = read_data_df()
    ...
    with col2:
        if st.button("💾 Skriv DATA"):
            try:
                cur = st.session_state.get("DATA")
                if cur is None or cur.empty:
                    st.warning("Ingen DATA i minnet att skriva.")
                else:
                    write_data_df(cur)
                    st.success("DATA skriven till Google Sheets.")
            except Exception as e:
                st.error(f"Kunde inte skriva DATA: {e}")

    if st.sidebar.button("🔁 Uppdatera FX"):
        try:
            _load_fx_and_update_sheet()
            st.session_state["FX"] = get_fx_map()
            st.success("Valutakurser uppdaterade.")
...
def main():
    ...
    # Standardvy: Analys
    default_key = "🔬 Analys"
    choice = st.sidebar.radio("Meny", list(PAGES.keys()), index=list(PAGES.keys()).index(default_key))

    # Liten statusrad om vald primärvaluta & FX
    try:
        s = get_settings_map()
        prim = s.get("primary_currency", "SEK")
        fx_map = st.session_state.get("FX", {}) or {}
        st.sidebar.caption(f"Primär valuta: **{prim}** · FX-par: **{len(fx_map)}**")
    except Exception:
        pass
