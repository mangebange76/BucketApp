...
def _infer_currency_from_price_df(df: pd.DataFrame) -> Optional[str]:
    # yfinance skriver ibland "Currency" i .info eller i .history metadata — saknas ofta.
    return None
    ...
    try:
        ...
        if isinstance(yfin, pd.DataFrame) and not yfin.empty and "Total Revenue" in yfin.index:
            ...
            if rev.shape[0] >= 5:
                ...
                rev_cagr = _cagr(start, end, 4)  # 5 år = 4 intervalldifferenser
    except Exception:
        pass
    ...
    try:
        ...
        if isinstance(yearn, pd.DataFrame) and not yearn.empty and "Earnings" in yearn.columns:
            ...
            if eps.shape[0] >= 5:
                ...
                # Om negativ baseline – CAGR blir meningslös. Lämna None i så fall.
...
def _fast_snapshot(ticker: str) -> Dict[str, Optional[float]]:
    ...
    # info-baserade saker (kan vara sega – använd sparsamt)
    try:
        inf = t.info
    except Exception:
        inf = {}
    ...
    try:
        ...
        # Om "lastDividendValue" är senaste kvartalsbelopp – multiplicera med frekvens om möjligt
        # Vi lämnar som är; bättre än över-aggressiv uppräkning.
    except Exception:
        pass
...
def fetch_from_yahoo(ticker: str) -> Dict[str, Any]:
    ...
    # Härleder PE TTM om Yahoo saknar trailingPE
    ...
    # Clamp enligt din praxis:
    #   • Revenue CAGR clamp: -10% .. +35%
    #   • EPS CAGR clamp:    -20% .. +35%  (negativ EPS -> None)
...
# -----------------------------
# Hjälpfunktion: sanera/merge in i Data-DF
# -----------------------------
...
def _ev_from_ebitda(ebitda: Optional[float], mult: Optional[float]) -> Optional[float]:
    # EBITDA kan vara <=0; då returnerar vi None om multipel saknas
    m = _pos(mult)
...
def _parse_date_any(x: Any) -> Optional[dt.date]:
    ...
    try:
        return pd.to_datetime(s, errors="coerce").date()  # type: ignore
    except Exception:
        return None
...
def _build_next_payouts_table(data_df: pd.DataFrame, fx_map: dict[str, float]) -> pd.DataFrame:
    ...
    for _, r in data_df.iterrows():
        ...
        if not _is_future_or_today(next_pay):
            # Hoppa över om datum saknas eller ligger i dåtid
            continue
...
def render_portfolio_view(data_df: pd.DataFrame, fx_map: dict[str, float]) -> None:
    ...
    # (inget tas bort; detta påverkar inte övriga vyer)
    ...
    with c3:
        # Enkel genomsnittlig "per månad" på nästa lista (illustrativt)
        m = df_next["Nästa utb.-datum"].dt.to_period("M").nunique() if not df_next.empty else 0
        per_m = (total_next_sek / m) if m else 0.0
...
#  • Säkerställ kolumner i DATA
#  • Auto-FX-refresh vid start (om valt i Settings)
#  • Sidopanel + router → sidor i Del 5
# ============================================================

# --- Små hjälpare (kompletterar Del 5) ---
def now_stamp() -> str:
    from datetime import datetime
    return datetime.now().strftime("%Y-%m-%d %H:%M:%S")

def _ensure_data_basics(df: pd.DataFrame) -> pd.DataFrame:
    """
    Säkrar att minst de kolumner som vyerna förlitar sig på finns.
    (Raderar inte något, lägger bara till saknade kolumner.)
    """
    must = [
        "Timestamp","Ticker","Bolagsnamn","Sektor","Bucket","Valuta",
        "Antal aktier","GAV (SEK)","Aktuell kurs","Utestående aktier",
        "EPS TTM","PE TTM","PE FWD","Rev TTM","EV/Revenue","EV/EBITDA",
    ...
    # inkludera huvudschemat om definierat
    try:
        for c in DATA_COLUMNS:
            if c not in must:
                must.append(c)
...
def _init_on_first_run():
    """Laddar Settings/FX/DATA till sessionen. Kör FX-refresh om aktiverad."""
    ss = st.session_state

    # Ladda Settings/FX från Sheets
    try:
    ...
    try:
        ...
        df = pd.DataFrame(columns=DATA_COLUMNS if 'DATA_COLUMNS' in globals() else [])
    df = _ensure_data_basics(df)
    ss["DATA"] = df

    # Auto-FX-refresh en gång (om aktiverat)
    auto = str(ss.get("SETTINGS", {}).get("auto_refresh_on_start", "0")) == "1"
    if auto and not ss.get("_FX_AUTO_RAN", False):
        try:
            _load_fx_and_update_sheet()
            ss["FX"] = get_fx_map()
            ss["_FX_AUTO_RAN"] = True
            st.toast("Valutakurser uppdaterade (auto vid start).")
        except Exception as e:
            st.warning(f"Kunde inte auto-uppdatera FX: {e}")

def _topbar_status():
    ss = st.session_state
    df = ss.get("DATA", pd.DataFrame())
    fx = ss.get("FX", {})
    s  = ss.get("SETTINGS", {})
    ...
    with c2:
        st.caption(f"💱 FX-valutor: **{len(fx)}**")
    with c3:
        st.caption(f"🌐 Primär valuta: **{s.get('primary_currency','SEK')}**")
    with c4:
        st.caption(f"⏱️ Senast uppdaterad: **{now_stamp()}**")
...
def main():
    ...
    with st.sidebar:
        ...
        page = st.radio(
            "Välj sida",
            list(PAGES.keys()),
            index=list(PAGES.keys()).index("Analys"),
        )
        st.markdown("---")
        if st.button("🔄 Ladda om DATA från Sheets"):
            try:
                ...
                df = _ensure_data_basics(df)
                st.session_state["DATA"] = df
                st.success("DATA inläst från Google Sheets.")
            except Exception as e:
                st.error(f"Kunde inte läsa DATA: {e}")

        if st.button("💾 Skriv nuvarande DATA → Google Sheets"):
            try:
                write_data_df(st.session_state.get("DATA", pd.DataFrame()))
                st.success("DATA sparat till Google Sheets.")
            except Exception as e:
                st.error(f"Kunde inte skriva DATA: {e}")

        st.markdown("---")
        if st.button("🔁 Uppdatera valutakurser (manuellt)"):
            try:
                _load_fx_and_update_sheet()
    ...
    # Kör vald sida
    try:
        PAGES[page]()  # anropa vy-funktionen
    except Exception as e:
