...
# ============================================================
# Del 6/6 — Huvudkörning & router
#  • Init av DATA/FX/Settings i session
#  • Sidopanel med vyval
#  • Säker felhantering runt vyer
# ============================================================

def _init_once():
    """Initiera sessionens cache: DATA, FX och Settings."""
    if "inited" not in st.session_state:
        st.session_state["inited"] = True

    # Ladda DATA (Google Sheets → Data-bladet) om saknas
    if "DATA" not in st.session_state or st.session_state.get("DATA") is None:
        try:
            ...
            st.sidebar.error(f"Kunde inte läsa DATA från Sheets: {e}")

    # Ladda FX-karta
    if "FX" not in st.session_state or not isinstance(st.session_state.get("FX"), Mapping):
        try:
    ...
    # Auto-refresh FX vid start om inställning satt (körs max en gång per session)
    s = get_settings_map()
    auto_refresh = str(s.get("auto_refresh_on_start", "0")) == "1"
    if auto_refresh and not st.session_state.get("fx_autorefreshed", False):
        try:
            _load_fx_and_update_sheet()
            st.session_state["FX"] = get_fx_map()
            st.session_state["fx_autorefreshed"] = True
        except Exception as e:
...
def _sidebar_menu() -> str:
    ...
    # Behåll valt läge i session
    if "page_key" not in st.session_state:
        st.session_state["page_key"] = "analysis"

    sel = st.sidebar.selectbox("Välj vy", options=page_labels,
                               index=page_labels.index(next(k for k,v in pages.items() if v == st.session_state["page_key"])))
    st.session_state["page_key"] = pages[sel]

    # Hjälpknappar
    if st.sidebar.button("🔄 Ladda om DATA från Sheets"):
        try:
            st.session_state["DATA"] = read_data_df()
            st.sidebar.success("DATA laddad från Google Sheets.")
        except Exception as e:
...
def _route(page_key: str):
    # Koppla till respektive vyfunktion (definierade i tidigare delar)
...
# Streamlit kör filen top-down; main() kallas för tydlighet
main()
