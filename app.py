...
#  - Använder Yahoo-hämtning (Del 2/Del 3) och beräkningsmotor (Del 3)
...
def build_buy_suggestions(df_data: pd.DataFrame, settings: Dict[str, str], fx_map: Dict[str, float],
    ...
    """
    Köpförslag (LIVE-FV):
      • Fair value beräknas per rad via compute_methods_for_row (samma som Analys-vyn).
      • Aktuell kurs = payload["price"] från beräkningsmotorn.
      ...
      • fv_horizon styr både:
          ...
          – köpsignalen (pris < vald FV).
    Övriga riktkurser används som visningsfält.
    """
    cols_out = [
    ...
    for _, r in base.iterrows():
        try:
            ...
            # 🔹 Beräkna fair value LIVE (samma som Analys-vyn)
            ...
            # Måste ha användbar FV för vald horisont
