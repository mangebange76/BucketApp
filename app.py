...
#  - Bootstrap av DATA/FX/Settings
#  - Sidopanel med navigering
#  - Lätt översiktssida
# ============================================================
...
def _bootstrap_session():
    try:
        ...
        st.warning(f"Kunde inte initiera: {e}")
...
def _refresh_all():
    ...
    st.session_state.pop("FX", None)
    st.session_state.pop("SETTINGS_MAP", None)
    st.session_state["SETTINGS_MAP"] = get_settings_map()
    st.session_state["FX"] = get_fx_map()
    st.session_state["DATA"] = read_data_df()
    st.success("Uppdaterat från Google Sheets.")
...
def page_overview():
    ...
    settings = st.session_state.get("SETTINGS_MAP", {})

    c1, c2, c3, c4 = st.columns(4)
    with c1:
        n_rows = 0 if df is None or df.empty else len(df)
        st.metric("Antal rader i Data", f"{n_rows}")
    with c2:
        n_tickers = 0 if df is None or df.empty else df["Ticker"].dropna().astype(str).str.upper().nunique()
        st.metric("Unika tickers", f"{n_tickers}")
    with c3:
        fx_cnt = len(fx or {})
        st.metric("Valutakurser i minnet", f"{fx_cnt}")
    with c4:
        st.metric("Tid", now_stamp())

    st.markdown("---")
    st.caption("Snabbtitt (första 10 raderna).")
    if df is None or df.empty:
...
def main():
    ...
    if st.sidebar.button("↻ Läs in från Sheets nu"):
        _refresh_all()
