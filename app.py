...
# ------------------------------------------------------------
# CHANGED: Hjälpfunktion för säker Data-inläsning från session
#           (undviker ValueError: DataFrame i bool-kontext)
#           Används i senare delar för t.ex. page_buy_suggestions.
# ------------------------------------------------------------
...
#  • render_rankingpanel(df, settings, fx_map)  ← kör först efter knapptryck
...
def render_rankingpanel(df: pd.DataFrame, settings: Dict[str, str], fx_map: Dict[str, float]) -> None:
    """
    Rankingpanel (körs först efter knapptryck):
    ...
    with right:
        st.caption("Kör endast när du klickar")
...
def page_settings():
    ...
    if st.button("💾 Spara Settings"):
        try:
            # Skriv endast kolumner enligt schemat
...
# ============================================================
# Editor (manuellt + Yahoo)  ✅ Bucket med rullista
# ============================================================
...
# ============================================================
# Lägg till ticker  ✅ Bucket med rullista
# ============================================================
def page_add_ticker():
...
def page_ranking():
    ...
    # --- Fallback (kompakt) — kräver knapptryck ---
...
#   • Fallback används om build_buy_suggestions() saknas
#   • Fix: ingen bool-ambiguitet på DataFrame
# ============================================================
...
def page_buy_suggestions():
    ...
    # ✅ Fix: undvik bool-ambiguitet för DataFrame
    df = st.session_state.get("DATA")
    if df is None or (isinstance(df, pd.DataFrame) and df.empty):
        df = read_data_df()
    if df is None or df.empty:
...
#  • Init av session (DATA, FX, SETTINGS_MAP)
#  • Sidebar-menyn
#  • Navigering till respektive vy
# ============================================================

def _init_session():
    ss = st.session_state
    # DATA
    if "DATA" not in ss or ss.get("DATA") is None:
        try:
    ...
    # FX (Valutakurser)
    if "FX" not in ss or ss.get("FX") is None:
        try:
            ss["FX"] = get_fx_map()
        except Exception:
            ss["FX"] = {}

    # Settings-map (nyladdad)
    try:
...
def _sidebar_menu() -> str:
    ...
    return st.sidebar.radio("Välj vy", pages, index=0)

def _route(page_name: str) -> None:
    if page_name == "📦 Portfölj":
        page_portfolio()
    elif page_name == "🔬 Analys":
        page_analysis()
    elif page_name == "🏆 Ranking":
        page_ranking()
    elif page_name == "🛒 Köpförslag":
        page_buy_suggestions()
    elif page_name == "✏️ Editor":
        page_editor()
    elif page_name == "➕ Lägg till":
        page_add_ticker()
    elif page_name == "🧩 Massuppdatering":
        page_batch()
