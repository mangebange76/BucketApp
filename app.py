...
def yahoo_fetch_for_ticker(sym: str) -> Dict[str, Any]:
    ...
    out: Dict[str, Any] = {
        ...
        "EV/EBITDA": _yf_ev_ebitda(tkr),   # <— korrekt multipel (inte EBITDA TTM)
        "P/B": _yf_p_b(tkr),
        "BVPS": _yf_bvps(tkr),
...
# ============================================================
# app.py — Aktieanalys & investeringsförslag
# Del 6/6: 🏆 Ranking + Navigation & main()
#  - Kör fair value-beräkning per rad (använder compute_methods_for_row)
#  - Skriver riktkurser (Idag/1/2/3 år) + Bull/Bear 1 år till Data-bladet
#  - Enkel status/progress och resultattabell
#  - Sidomeny som kopplar ihop alla vyer
# ============================================================
...
def _col(df: pd.DataFrame, *names: str) -> Optional[str]:
    """Returnerar första kolumn som finns bland aliasen."""
    for n in names:
        if n in df.columns:
            return n
    return None

def _ensure_cols_for_targets(df: pd.DataFrame) -> pd.DataFrame:
    needed = [
        "Riktkurs idag", "Riktkurs 1 år", "Riktkurs 2 år", "Riktkurs 3 år",
        "Bull 1 år", "Bear 1 år", "Senast rankad"
    ]
    for c in needed:
        if c not in df.columns:
            df[c] = np.nan
...
def _extract_targets_from_methods(methods_df: pd.DataFrame) -> Dict[str, Optional[float]]:
    """
    Förväntad 'methods_df' från compute_methods_for_row:
      - En rad 'fair_value' (metodfamilj-median) med targets för today/1y/2y/3y
      - Ev. bull_1y/bear_1y
    Vi försöker vara robusta mot kolumnnamn.
    """
    if methods_df is None or methods_df.empty:
        return {
            "today": None, "y1": None, "y2": None, "y3": None,
            "bull_1y": None, "bear_1y": None
        }

    # Lokalisera fair_value-raden
    row = None
    if "method" in methods_df.columns:
        m = methods_df[methods_df["method"].astype(str).str.lower() == "fair_value"]
        if not m.empty:
            row = m.iloc[0]
    if row is None and "Metod" in methods_df.columns:
        m = methods_df[methods_df["Metod"].astype(str).str.lower() == "fair_value"]
        if not m.empty:
            row = m.iloc[0]
    if row is None and "name" in methods_df.columns:
        m = methods_df[methods_df["name"].astype(str).str.lower() == "fair_value"]
        if not m.empty:
            row = m.iloc[0]
    if row is None:
        # Om ingen fair_value-rad – försök plocka median över familjer om sådan kolumn finns
        # (Fallback: ta median över alla metodrader för varje horisont.)
        def _safe_med(col_aliases):
            for ca in col_aliases:
                if ca in methods_df.columns:
                    s = pd.to_numeric(methods_df[ca], errors="coerce")
                    s = s.dropna()
                    if not s.empty:
        ...
        return {
            "today": _safe_med(["target_today","Fair value idag","Riktkurs idag","Idag"]),
            "y1":    _safe_med(["target_1y","Fair value 1y","Riktkurs 1 år","1 år"]),
            "y2":    _safe_med(["target_2y","Fair value 2y","Riktkurs 2 år","2 år"]),
            "y3":    _safe_med(["target_3y","Fair value 3y","Riktkurs 3 år","3 år"]),
            "bull_1y": _safe_med(["bull_1y","Bull 1 år"]),
            "bear_1y": _safe_med(["bear_1y","Bear 1 år"]),
        }

    def g(series, *aliases):
        for a in aliases:
            if a in series and _f(series[a]) is not None:
                return float(_f(series[a]))
        return None

    return {
        "today":   g(row, "target_today", "Fair value idag", "Riktkurs idag", "Idag"),
        "y1":      g(row, "target_1y",    "Fair value 1y",   "Riktkurs 1 år", "1 år"),
        "y2":      g(row, "target_2y",    "Fair value 2y",   "Riktkurs 2 år", "2 år"),
        "y3":      g(row, "target_3y",    "Fair value 3y",   "Riktkurs 3 år", "3 år"),
        "bull_1y": g(row, "bull_1y",      "Bull 1 år"),
        "bear_1y": g(row, "bear_1y",      "Bear 1 år"),
    }

def _apply_targets_to_df(data_df: pd.DataFrame, idx: int, t: Dict[str, Optional[float]]) -> None:
    data_df.at[idx, "Riktkurs idag"] = t.get("today")
    data_df.at[idx, "Riktkurs 1 år"]  = t.get("y1")
    data_df.at[idx, "Riktkurs 2 år"]  = t.get("y2")
    data_df.at[idx, "Riktkurs 3 år"]  = t.get("y3")
    if t.get("bull_1y") is not None:
        data_df.at[idx, "Bull 1 år"] = t.get("bull_1y")
    if t.get("bear_1y") is not None:
        data_df.at[idx, "Bear 1 år"] = t.get("bear_1y")
    data_df.at[idx, "Senast rankad"] = now_stamp()

# =========================
# 🏆 Ranking – UI
# =========================
def page_ranking():
    st.header("🏆 Ranking (fair value → Data-bladet)")

    base_df = st.session_state.get("DATA")
    ...
    df = _ensure_cols_for_targets(base_df.copy())

    # Välj vilka tickers som ska köras
    all_tickers = sorted(df["Ticker"].dropna().astype(str).unique().tolist())
    sel = st.multiselect("Välj tickers (tom = alla)", options=all_tickers, default=[])

    # Filter: kör bara på bolag där vi har minst basdata (price/shares/bvps etc behövs av motorn)
    st.caption("Tips: se till att pris, valuta, aktier, nettoskuld och TTM-nycklar finns i raden.")
    go = st.button("🚀 Kör ranking")

    if not go:
        # Visa snabb sammanfattning av nuvarande riktkurser
        show = df[["Ticker","Bolagsnamn","Valuta","Aktuell kurs","Riktkurs idag","Riktkurs 1 år","Riktkurs 2 år","Riktkurs 3 år","Bull 1 år","Bear 1 år"]].copy()
        st.dataframe(show, use_container_width=True, hide_index=True)
        return

    target = all_tickers if len(sel) == 0 else [s.upper() for s in sel]
    ...
    for i, tkr in enumerate(target, start=1):
        try:
            ...
            row = df.loc[ridx].copy()

            # === KÄRNA: anropa beräkningsmotorn från Del 3 ===
            # Förväntat: compute_methods_for_row(row) → (methods_df, meta) eller (methods_df, sanity, meta)
            try:
                result = compute_methods_for_row(row)  # definierad tidigare (Del 3)
            except TypeError:
                # Om signaturen är (row, settings) i din bas – passa vidare settings
                result = compute_methods_for_row(row, get_settings_map())

            # Packa upp returvärde robust
            methods_df = None
            meta = {}
            if isinstance(result, tuple):
                if len(result) == 3:
                    methods_df, _, meta = result
                elif len(result) == 2:
                    methods_df, meta = result
                else:
                    methods_df = result[0]
            else:
                methods_df = result

            targets = _extract_targets_from_methods(methods_df if isinstance(methods_df, pd.DataFrame) else pd.DataFrame())
            _apply_targets_to_df(df, ridx, targets)

            rows_out.append({
                "Ticker": tkr,
                "Valuta": str(_nz(row.get("Valuta"), "")),
                "Kurs": _f(row.get("Aktuell kurs")),
                "Riktkurs idag": targets.get("today"),
                "Riktkurs 1 år":  targets.get("y1"),
                "Riktkurs 2 år":  targets.get("y2"),
                "Riktkurs 3 år":  targets.get("y3"),
                "Bull 1 år":      targets.get("bull_1y"),
                "Bear 1 år":      targets.get("bear_1y"),
            })
        except Exception as e:
            st.error(f"{tkr}: {e}")
        finally:
    ...
    try:
        write_data_df(df)
        st.session_state["DATA"] = df
        st.success(f"Klar. {len(rows_out)} tickers uppdaterade i Data-bladet.")
    except Exception as e:
    ...
    if rows_out:
        ...
        # Formatering
        for c in ("Kurs","Riktkurs idag","Riktkurs 1 år","Riktkurs 2 år","Riktkurs 3 år","Bull 1 år","Bear 1 år"):
            if c in out.columns:
...
def main():
    try:
        _boot()  # definierad i Del 1 – läser Settings/FX/Data till session cache
    except Exception as e:
        st.error(f"Initfel: {e}")
...
# Streamlit kör main() när filen laddas
main()
