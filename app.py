...
def _eps_path(eps_ttm: Optional[float], eps_1y: Optional[float], eps_2y: Optional[float], eps_cagr: Optional[float]) -> Tuple[Optional[float], Optional[float], Optional[float], Optional[float]]:
    ...
    • EPS 2Y hämtas automatiskt från Finnhub om möjligt.
...
def compute_methods_for_row(row: pd.Series, settings: Dict[str, str], fx_map: Dict[str, float]) -> Tuple[pd.DataFrame, str, Dict[str, Any]]:
    ...
    # 4) Paths (EPS 2Y fylls automatiskt om möjligt via Finnhub eller CAGR)
    e0, e1, e2, e3 = _eps_path(eps_ttm, eps_1y_est, eps_2y_est, eps_cagr)
    r0, r1, r2, r3 = _rev_path(rev_ttm, rev_cagr)

    # EBITDA följer omsättningsbanan som proxy om vi saknar specifika estimat
...
# -------------------------
# Yahoo – hämta ÅRS-historik (Total Revenue) och härled Rev CAGR
# -------------------------
def _extract_total_revenue_series(df: pd.DataFrame) -> List[float]:
    """Plockar ut en stigande tidsserie av 'Total Revenue' från en Yahoo-DataFrame (årsdata)."""
    if not isinstance(df, pd.DataFrame) or df.empty:
        return []
    # försök hitta rätt rad
    row = None
    for idx in df.index:
        s = str(idx).strip().lower().replace(" ", "")
        if s in ("totalrevenue", "revenuetotal", "revenue"):
            row = df.loc[idx]
            break
    if row is None:
        return []
    # bygg (datum, värde) och sortera på datum
    pairs = []
    for col in row.index:
        ...
        try:
            ...
            dt = None
        pairs.append((dt, val))
    pairs.sort(key=lambda x: (x[0] is None, x[0]))
    return [v for _, v in pairs if v > 0]

@st.cache_data(ttl=3600, show_spinner=False)
def fetch_yahoo_rev_history_and_cagr(ticker: str) -> Dict[str, Any]:
    """
    Försöker hämta årsvis 'Total Revenue' och räkna ut Rev CAGR (3–5 års fönster om möjligt).
    Returnerar: {'rev_series': [...ascending...], 'rev_cagr': float|None, 'source': 'yahoo_financials'|'none'}
    """
    tk = yf.Ticker(ticker)
    series: List[float] = []

    # Nya/äldre yfinance-vägar – prova flera
    for attr in ("income_stmt", "financials"):
        try:
            df = getattr(tk, attr, None)
            if isinstance(df, pd.DataFrame) and not df.empty:
                series = _extract_total_revenue_series(df)
                if series:
                    break
        except Exception:
            pass
    # Vissa versioner har get_income_stmt(freq='annual')
    if not series:
        try:
            get_fn = getattr(tk, "get_income_stmt", None)
            if callable(get_fn):
                df = get_fn(freq="annual")
                if isinstance(df, pd.DataFrame) and not df.empty:
                    series = _extract_total_revenue_series(df)
        except Exception:
            pass
    ...
    if not series or len(series) < 3:
        return {"rev_series": series or [], "rev_cagr": None, "source": "none"}

    # Ta ett fönster om upp till 5 senaste års värden
    window = series[-5:] if len(series) > 5 else series
    first, last = window[0], window[-1]
    n_years = max(1, len(window) - 1)
    try:
        cagr = (last / first) ** (1.0 / n_years) - 1.0
    except Exception:
        cagr = None

    return {"rev_series": window, "rev_cagr": cagr, "source": "yahoo_financials"}
...
def compute_methods_for_row(row: pd.Series, settings: Dict[str, str], fx_map: Dict[str, float]) -> Tuple[pd.DataFrame, str, Dict[str, Any]]:
    ...
    # Estimat / tillväxt – EPS
    eps_1y_est = _pos(_nz(est.get("eps_1y"), row.get("EPS 1Y")))
    eps_2y_est = _pos(_nz(est.get("eps_2y"), row.get("EPS 2Y")))
    eps_cagr   = _f(row.get("EPS CAGR"))

    # Om EPS CAGR saknas/0 men vi har eps_ttm & eps_1y -> enkel CAGR för ett år
    if (eps_cagr is None or abs(eps_cagr) < 1e-9) and _pos(eps_ttm) and _pos(eps_1y_est):
        try:
            eps_cagr = (float(eps_1y_est) / float(eps_ttm)) - 1.0
        except Exception:
            eps_cagr = None

    # 2b) Rev CAGR från Yahoo om saknas/0 – och fyll rev_ttm om saknas
    rev_cagr = _f(row.get("Rev CAGR"))
    rev_cagr_source = "sheet"
    if rev_cagr is None or abs(rev_cagr) < 1e-9:
        rh = fetch_yahoo_rev_history_and_cagr(ticker)
        if rev_cagr is None or abs(rev_cagr) < 1e-9:
            rev_cagr = rh.get("rev_cagr")
            if rev_cagr is not None:
                rev_cagr_source = rh.get("source", "yahoo_financials")
        # om vi saknar rev_ttm, använd senaste års-värdet
        if rev_ttm is None:
            try:
                s = rh.get("rev_series") or []
                if s:
                    rev_ttm = float(s[-1])
            except Exception:
                pass
    ...
    sanity = (
        ...
        f"rev_ttm={'ok' if r0 else '—'}({src.get('revenue_ttm','?')}), "
        f"rev_cagr={round(rev_cagr,4) if rev_cagr is not None else '—'}({rev_cagr_source}), "
        f"ebitda_ttm={'ok' if b0 else '—'}({src.get('ebitda_ttm','?')}), "
        f"shares={'ok' if shares else '—'}({src.get('shares','?')}), "
        f"pe_anchor={round(pe_anchor,2) if pe_anchor else '—'}, decay={decay}"
    )
    ...
    meta = {
        ...
        "sources": {
            **src,
            ...
            "eps_2y_source": "finnhub" if est.get("source") == "finnhub" and _pos(est.get("eps_2y")) else "sheet/derived",
            "rev_cagr_source": rev_cagr_source,
        },
        "eps_path": {"ttm": e0, "y1": e1, "y2": e2, "y3": e3},
        "rev_path": {"ttm": r0, "y1": r1, "y2": r2, "y3": r3},
        "ebitda_path": {"ttm": b0, "y1": b1, "y2": b2, "y3": b3},
...
def _company_card(row: pd.Series, settings: Dict[str, str], fx_map: Dict[str, float]) -> Tuple[Optional[str], Optional[float], Optional[float], Optional[float], Optional[float], Dict[str, Any]]:
    ...
    if b3.button("♻️ Uppdatera estimat/CAGR i Data", key=f"upd_est_{tkr}"):
        ...
        if mask.any():
            ...
            if "EPS 2Y" not in df.columns: df["EPS 2Y"] = np.nan
            if "EPS CAGR" not in df.columns: df["EPS CAGR"] = np.nan
            if _pos(e1): df.loc[mask, "EPS 1Y"] = float(e1)
            if _pos(meta.get("eps_path", {}).get("y2")):
                df.loc[mask, "EPS 2Y"] = float(meta["eps_path"]["y2"])
...
def page_editor():
    ...
    # Session-state för prefill (påverkas av "Hämta & fyll")
    ...
    if fetch_btn:
        if not ticker:
            ...
            st.stop()
        snap = fetch_yahoo_snapshot(ticker)
        # Försök även hämta EPS-estimat (1–2 år) via Finnhub (om nyckel finns)
        est = fetch_finnhub_estimates(ticker)
        st.session_state["editor_prefill"] = {
            "Ticker": ticker,
            "Bolagsnamn": _nz(name, None),
            "Sektor": _nz(sector, None),
            "Bucket": _nz(bucket, None),
    ...
    if save_btn:
        ...
        new_row = {
            ...
            "EPS 2Y": eps2y,
            "Rev CAGR": revcg,
            "EPS CAGR": _f(merged.get("EPS CAGR")),  # kan räknas om i Analys
            "Årlig utdelning": dps,
