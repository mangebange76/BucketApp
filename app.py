...
# ============================================================
# app.py — Aktieanalys & investeringsförslag
# Del 3/6: Riktkurser & Fair Value (metodfamiljer + median)
#
#  - clamp av CAGR (Rev: [-10%, +35%], EPS: [-20%, +35%])
#  - PE-ankare = viktad mix av PE TTM/FWD (pe_anchor_weight_ttm default 0.50)
#  - Multipel-decay per år (default 0.10) med golv 60% av startmultipeln
#  - Paths: Revenue, EPS, EBITDA (EBITDA ~ Rev via marginal)
#  - Metoder: pe_hist_vs_eps, ev_sales, ev_ebitda, p_b
#  - Fair Value V2: median över metodfamiljer (pe, ev_s, ev_e, pb)
#    • I "Idag": filtrera bort värden inom ±0.5% av aktuell kurs
#    • Fallback till pe-raden om allt filtreras bort
#  - Returnerar (methods_df, meta)
# ============================================================

# ------------------------------
# Hjälpfunktioner (säkra läsningar)
# ------------------------------
def _get_num(row: Mapping, key: str, default: Optional[float] = None) -> Optional[float]:
    try:
        v = row.get(key) if isinstance(row, Mapping) else row[key]
        if v is None:
            return default
        v = float(v)
...
def _decay_mult(years: int, start_mult: float, decay: float, floor_frac: float) -> float:
    """
    År n får multipeln = max(floor_frac*start, start * (1-decay)^n).
    """
    if start_mult is None or not math.isfinite(start_mult) or start_mult <= 0:
        return float("nan")
    floor_val = start_mult * floor_frac
    cand = start_mult * ((1.0 - decay) ** years)
    return max(floor_val, cand)

def _safe_growth_series(v0: Optional[float], g: Optional[float], years: int) -> List[Optional[float]]:
    """
    Geometrisk tillväxt från v0 med årlig g under 'years' år.
    Returnerar [v0, v1, v2, v3] upp till 3 år (eller så många som behövs).
    """
    out = [None] * (years + 1)
    out[0] = v0 if (v0 is not None and math.isfinite(v0)) else None
    if out[0] is None:
        return out
    gg = g if (g is not None and math.isfinite(g)) else 0.0
    cur = out[0]
...
def _derive_cagr(v0: Optional[float], vN: Optional[float], years: int) -> Optional[float]:
    """
    CAGR från v0 till vN över 'years' år.
    """
    try:
        if v0 is None or vN is None or v0 <= 0 or not math.isfinite(v0) or not math.isfinite(vN) or years <= 0:
            return None
...
def compute_methods_for_row(
    row: pd.Series,
    pe_anchor_weight_ttm: float = 0.50,
    multiple_decay: float = 0.10,
    multiple_floor: float = 0.60,
) -> Tuple[pd.DataFrame, Dict[str, Any]]:
    """
    Beräknar riktkurser via flera metoder och sammanställer 'fair_value' som median
    över metodfamiljer (pe, ev_s, ev_e, pb). Allt i bolagets handelsvaluta.
    """

    # --- Inputs från rad ---
    price_now   = _get_num(row, "Aktuell kurs")
    currency    = str(row.get("Valuta", "") or "").upper()

    shares_out  = _get_num(row, "Utestående aktier")
    net_debt    = _get_num(row, "Net debt", 0.0)  # kan vara negativ (nettokassa)

    eps_ttm     = _get_num(row, "EPS TTM")
    pe_ttm      = _get_num(row, "PE TTM")
    pe_fwd      = _get_num(row, "PE FWD")

    rev_ttm     = _get_num(row, "Rev TTM")
    ebitda_ttm  = _get_num(row, "EBITDA TTM")
    ev_rev      = _get_num(row, "EV/Revenue")
    ev_ebitda   = _get_num(row, "EV/EBITDA")

    p_b         = _get_num(row, "P/B")
    bvps        = _get_num(row, "BVPS")

    # Estimat / historik för härledning av tillväxt
    rev_1y      = _get_num(row, "Rev 1Y")
    rev_2y      = _get_num(row, "Rev 2Y")
    eps_1y      = _get_num(row, "EPS 1Y")
    eps_2y      = _get_num(row, "EPS 2Y")
    rev_cagr_in = _get_num(row, "Rev CAGR")
    eps_cagr_in = _get_num(row, "EPS CAGR")

    # --- Fallback för aktuell kurs om saknas ---
    if (price_now is None or not math.isfinite(price_now)) and (pe_ttm is not None and eps_ttm is not None):
        # Enkel fallback: P/E TTM * EPS TTM
        try:
            price_now = float(pe_ttm) * float(eps_ttm)
        except Exception:
            price_now = None

    # --- PE-ankare ---
    # Blandning av PE TTM / PE FWD (om FWD saknas, använd TTM; om TTM saknas, använd FWD)
    if pe_ttm is None and pe_fwd is None:
        pe_anchor = None
    else:
        w_ttm = float(pe_anchor_weight_ttm)
        if pe_ttm is None:
            pe_anchor = pe_fwd
        elif pe_fwd is None:
            pe_anchor = pe_ttm
        else:
            pe_anchor = w_ttm * pe_ttm + (1.0 - w_ttm) * pe_fwd

    # --- CAGR valet + clamp ---
    # Revenue CAGR: föredra explicit Rev CAGR, annars härled från Rev TTM → Rev 2Y (2 år fram)
    rev_cagr_raw = rev_cagr_in
    if rev_cagr_raw is None:
        rev_cagr_raw = _derive_cagr(rev_ttm, rev_2y, 2) or _derive_cagr(rev_ttm, rev_1y, 1)
    rev_cagr = _clamp(rev_cagr_raw, -0.10, 0.35)  # [-10%, +35%]

    # EPS CAGR: föredra explicit EPS CAGR, annars härled från EPS TTM → EPS 2Y
    eps_cagr_raw = eps_cagr_in
    if eps_cagr_raw is None:
        eps_cagr_raw = _derive_cagr(eps_ttm, eps_2y, 2) or _derive_cagr(eps_ttm, eps_1y, 1)
    eps_cagr = _clamp(eps_cagr_raw, -0.20, 0.35)  # [-20%, +35%]

    # Regel: EPS-path använder g i ordning: eps_cagr → (om None) rev_cagr → (om None) 0
    g_eps = eps_cagr if eps_cagr is not None else (rev_cagr if rev_cagr is not None else 0.0)

    # --- Paths (0..3 år) ---
    # Revenue
    rev_path = _safe_growth_series(rev_ttm, rev_cagr if rev_cagr is not None else 0.0, 3)  # [R0..R3]
    # EPS
    eps_path = _safe_growth_series(eps_ttm, g_eps, 3)  # [E0..E3]
    # EBITDA ~ Rev via marginal
    ebitda_path: List[Optional[float]]
    if ebitda_ttm is not None and rev_ttm and rev_ttm > 0:
        margin = ebitda_ttm / rev_ttm
        ebitda_path = [None, None, None, None]
        for i in range(0, 4):
            Ri = rev_path[i] if i < len(rev_path) else None
            ebitda_path[i] = (margin * Ri) if (Ri is not None) else None
    else:
        ebitda_path = [None, None, None, None]

    # ------------------------------
    # Metoder (per horisont)
    # ------------------------------
    methods = []

    def _price_from_ev(ev_val: Optional[float]) -> Optional[float]:
        if ev_val is None or shares_out is None or not math.isfinite(ev_val) or not math.isfinite(shares_out) or shares_out <= 0:
            return None
        nd = net_debt if (net_debt is not None and math.isfinite(net_debt)) else 0.0
        eq = ev_val - nd
        if not math.isfinite(eq):
            return None
        return max(0.0, eq / shares_out)

    # --- PE mot EPS-path (familj "pe") ---
    if pe_anchor is not None and eps_path[0] is not None:
        row_pe = {"method": "pe_hist_vs_eps", "family": "pe"}
        for n in range(0, 4):
            eps_n = eps_path[n]
            if eps_n is None:
                row_pe[{0: "today", 1: "1y", 2: "2y", 3: "3y"}[n]] = None
                continue
            mult_n = _decay_mult(n, pe_anchor, multiple_decay, multiple_floor)
            px_n = eps_n * mult_n if (math.isfinite(mult_n)) else None
            row_pe[{0: "today", 1: "1y", 2: "2y", 3: "3y"}[n]] = px_n
        methods.append(row_pe)

    # --- EV/Sales (familj "ev_s") ---
    if ev_rev is not None and rev_path[0] is not None:
        row_es = {"method": "ev_sales", "family": "ev_s"}
        for n in range(0, 4):
            Rn = rev_path[n]
            if Rn is None:
                row_es[{0: "today", 1: "1y", 2: "2y", 3: "3y"}[n]] = None
                continue
            mult_n = _decay_mult(n, ev_rev, multiple_decay, multiple_floor)
            ev_n = Rn * mult_n if (math.isfinite(mult_n)) else None
            row_es[{0: "today", 1: "1y", 2: "2y", 3: "3y"}[n]] = _price_from_ev(ev_n)
        methods.append(row_es)

    # --- EV/EBITDA (familj "ev_e") ---
    if ev_ebitda is not None and ebitda_path[0] is not None:
        row_ee = {"method": "ev_ebitda", "family": "ev_e"}
        for n in range(0, 4):
            En = ebitda_path[n]
            if En is None:
                row_ee[{0: "today", 1: "1y", 2: "2y", 3: "3y"}[n]] = None
                continue
            mult_n = _decay_mult(n, ev_ebitda, multiple_decay, multiple_floor)
            ev_n = En * mult_n if (math.isfinite(mult_n)) else None
            row_ee[{0: "today", 1: "1y", 2: "2y", 3: "3y"}[n]] = _price_from_ev(ev_n)
        methods.append(row_ee)

    # --- P/B (familj "pb") ---
    if p_b is not None and bvps is not None:
        row_pb = {"method": "p_b", "family": "pb"}
        for n in range(0, 4):
            mult_n = _decay_mult(n, p_b, multiple_decay, multiple_floor)
            px_n = (mult_n * bvps) if (bvps is not None and math.isfinite(mult_n)) else None
            row_pb[{0: "today", 1: "1y", 2: "2y", 3: "3y"}[n]] = px_n
        methods.append(row_pb)

    # Gör DataFrame
    methods_df = pd.DataFrame(methods, columns=["method", "family", "today", "1y", "2y", "3y"]).dropna(how="all")
    # Fair value via median över familjer per horisont
    families = ("pe", "ev_s", "ev_e", "pb")

    def _median_over_families(h: str, filter_price_now: bool) -> Optional[float]:
        vals = []
        for fam in families:
            sub = methods_df[methods_df["family"] == fam]
            if sub.empty:
                continue
            v = sub[h].dropna().astype(float)
            if v.empty:
                continue
            # Ta median inom familjen (om flera metoder i samma familj i framtiden)
            fam_med = float(v.median())
            vals.append(fam_med)
        if not vals:
            return None
        # I "today": filtrera bort ~aktuell kurs (±0.5%)
        if filter_price_now and price_now is not None and math.isfinite(price_now) and price_now > 0:
            lo = price_now * 0.995
            hi = price_now * 1.005
            vals = [x for x in vals if not (lo <= x <= hi)]
            if not vals:
                # Fallback: om allt filtrerades bort, försök pe-raden
                sub_pe = methods_df[methods_df["family"] == "pe"]
                if not sub_pe.empty and pd.notna(sub_pe[h].iloc[0]):
                    return float(sub_pe[h].iloc[0])
                return None
        return float(pd.Series(vals).median())

    fv_today = _median_over_families("today", filter_price_now=True)
    fv_1y    = _median_over_families("1y",    filter_price_now=False)
    fv_2y    = _median_over_families("2y",    filter_price_now=False)
    fv_3y    = _median_over_families("3y",    filter_price_now=False)

    # Lägg till fair_value-rad längst ned
    fv_row = {
        "method": "fair_value",
        "family": "fair",
        "today": fv_today,
    ...
    if methods_df.empty:
        methods_df = pd.DataFrame([fv_row], columns=["method", "family", "today", "1y", "2y", "3y"])
    else:
        methods_df = pd.concat([methods_df, pd.DataFrame([fv_row])], ignore_index=True)

    # Bygg meta för insyn/debug
    meta = {
        ...
        "net_debt": net_debt,
        "pe_anchor": pe_anchor,
        "multiple_decay": multiple_decay,
        "multiple_floor": multiple_floor,
        "rev_cagr_raw": rev_cagr_raw,
        "rev_cagr": rev_cagr,
        "eps_cagr_raw": eps_cagr_raw,
        "eps_cagr": eps_cagr,
        "paths": {
            "revenue": rev_path,  # [R0..R3]
            "eps": eps_path,      # [E0..E3]
            "ebitda": ebitda_path # [Ebitda0..3]
        },
        "fair_value": {
            "today": fv_today,
            "1y": fv_1y,
...
# ============================================================
# app.py — Aktieanalys & investeringsförslag
# Del 4/6: Fair value-integration till databasen
#
#  - Säker läs/skriv av målkolumner (Riktkurs idag/1y/2y/3y)
#  - Använder compute_methods_for_row (Del 3/6)
#  - Två decimaler på riktkurser
#  - Fyller "Aktuell kurs" om den saknas (fallback från Del 3/6)
# ============================================================

# Kolumnnamn som används i basappen
_TARGET_COLS = ["Riktkurs idag", "Riktkurs 1 år", "Riktkurs 2 år", "Riktkurs 3 år"]
_PRICE_COL   = "Aktuell kurs"
_CCY_COL     = "Valuta"
...
def _ensure_target_columns(df: pd.DataFrame) -> pd.DataFrame:
    """Se till att riktkurskolumnerna finns; skapa dem om de saknas."""
    for c in _TARGET_COLS:
        if c not in df.columns:
            df[c] = None
    return df

def _assign_targets_to_row(df: pd.DataFrame, idx, fv: Dict[str, Optional[float]]) -> None:
    """Skriv fair value -> riktkurskolumner (två decimaler)."""
    mapping = {
        "today": "Riktkurs idag",
        "1y":    "Riktkurs 1 år",
        "2y":    "Riktkurs 2 år",
    ...
    for k_src, k_dst in mapping.items():
        df.at[idx, k_dst] = _round2(fv.get(k_src))
...
def compute_targets_for_row(
    ...
    """
    Beräkna fair value för en enskild rad och returnera {'today','1y','2y','3y'}.
    Kapslar compute_methods_for_row (Del 3/6).
    """
    methods_df, meta = compute_methods_for_row(
        row,
        pe_anchor_weight_ttm=pe_anchor_weight_ttm,
        multiple_decay=multiple_decay,
        multiple_floor=multiple_floor,
    )
    fv = (meta or {}).get("fair_value", {}) or {}
    # Returnera råa floats (rundning sker vid skrivning)
    return {
        "today": fv.get("today"),
        "1y":    fv.get("1y"),
        "2y":    fv.get("2y"),
...
def recalc_fair_values_for_df(
    ...
    *,
    ...
    """
    Beräkna och skriv fair value (riktkurser) för hela DataFrame:en.
    - Lägger till saknade målkolumner
    - Skriver två decimaler
    - Fyller 'Aktuell kurs' om saknas och kan härledas
    """
    if df is None or len(df) == 0:
        return df

    out = df if in_place else df.copy(deep=True)
    _ensure_target_columns(out)

    # Iterera över rader och uppdatera
    for idx, row in out.iterrows():
        try:
            # Beräkna FV
            fv = compute_targets_for_row(
                row,
                pe_anchor_weight_ttm=pe_anchor_weight_ttm,
                multiple_decay=multiple_decay,
                multiple_floor=multiple_floor,
            )
            _assign_targets_to_row(out, idx, fv)

            # Om 'Aktuell kurs' saknas: försök fylla från compute_methods_for_row-fallback
            # Gör en minimal körning igen för att hämta meta.price_now utan att duplicera logik
            methods_df, meta = compute_methods_for_row(
                row,
                pe_anchor_weight_ttm=pe_anchor_weight_ttm,
                multiple_decay=multiple_decay,
            ...
            price_now = (meta or {}).get("price_now", None)
            if (out.at[idx, _PRICE_COL] is None or (isinstance(out.at[idx, _PRICE_COL], float) and not math.isfinite(out.at[idx, _PRICE_COL]))) and price_now is not None:
                out.at[idx, _PRICE_COL] = _round2(price_now)

            # Valuta: säkerställ att det är sträng (ingen modifikation i sak – bara sanering)
            if _CCY_COL in out.columns:
                ccy = row.get(_CCY_COL, None)
                out.at[idx, _CCY_COL] = (str(ccy).upper() if ccy is not None else "")

        except Exception:
            # Skydda huvudloopen: skriv None om något går snett för den raden
            _assign_targets_to_row(out, idx, {"today": None, "1y": None, "2y": None, "3y": None})
            # Lämna 'Aktuell kurs' orörd vid fel
...
#  - Använder Yahoo-hämtning (Del 2/3) och beräkningsmotor (Del 3)
...
# ============================================================
# app.py — Aktieanalys & investeringsförslag
# Del 6/6: Analys, Ranking (Fair Value) & main()
#
#  - 🔍 Analys: visa enskilt bolag, fair value, metoder och nycklar
#  - 🏆 Ranking: räkna fram riktkurser (Idag/1y/2y/3y) och spara till Data
#  - 🧭 Huvudmeny & router (sidebar)
#
#  OBS: Förutsätter att Del 1–5 laddats (helpers, I/O, Yahoo, motor mm).
# ============================================================

# ------------------------------------------------------------
# Fallback: tydligt fel om beräkningsfunktionen saknas
# ------------------------------------------------------------
if "compute_methods_for_row" not in globals():
    def compute_methods_for_row(*args, **kwargs):
        raise RuntimeError(
            "compute_methods_for_row saknas. Säkerställ att Del 3/6 (beräkningsmotor) är inläst "
            "och att funktionen exporteras som 'compute_methods_for_row(row, ...)'."
        )

# ------------------------------------------------------------
# Robust extraktion av fair value från motor-output
# ------------------------------------------------------------
def _extract_targets_from_result(methods_df: pd.DataFrame | None, meta: dict | None) -> dict:
    out = {"today": None, "1y": None, "2y": None, "3y": None, "bull_1y": None, "bear_1y": None}
    try:
        if methods_df is not None and not methods_df.empty:
            idx = [str(i).lower() for i in getattr(methods_df, "index", [])]
            if "fair_value" in idx:
                fr = methods_df.iloc[idx.index("fair_value")]
                # Prova flera kolumnnamn
                candidates = {
                    "today": ["target_today","today","Idag","FV Idag","Fair Today"],
                    "1y":    ["target_1y","1y","1 år","FV 1 år","Fair 1Y"],
                    "2y":    ["target_2y","2y","2 år","FV 2 år","Fair 2Y"],
                    "3y":    ["target_3y","3y","3 år","FV 3 år","Fair 3Y"],
                    "bull_1y":["bull_1y","Bull 1 år","Bull1Y"],
                    "bear_1y":["bear_1y","Bear 1 år","Bear1Y"],
                }
                for k, cols in candidates.items():
                    for c in cols:
                        if c in methods_df.columns and _f(fr.get(c)) is not None:
                            out[k] = float(_f(fr.get(c)))
                            break
        # Meta fallback
        if meta:
            # Vanliga nycklar
            meta_map = {
                "today": ["fair_value_today","fv_today","today"],
                "1y":    ["fair_value_1y","fv_1y","1y"],
                "2y":    ["fair_value_2y","fv_2y","2y"],
                "3y":    ["fair_value_3y","fv_3y","3y"],
                "bull_1y":["bull_1y","bull"], "bear_1y":["bear_1y","bear"]
            }
            for k, keys in meta_map.items():
                if out[k] is None:
                    for key in keys:
                        ...
                        if isinstance(v, dict) and "value" in v:
                            v = v.get("value")
                        if _f(v) is not None:
                            out[k] = float(_f(v)); break
            # Ibland ligger fair_value som dict
            fv = meta.get("fair_value") if isinstance(meta, dict) else None
            if isinstance(fv, dict):
                out["today"] = out["today"] or _f(fv.get("today"))
                out["1y"]    = out["1y"]    or _f(fv.get("1y"))
                out["2y"]    = out["2y"]    or _f(fv.get("2y"))
                out["3y"]    = out["3y"]    or _f(fv.get("3y"))
    except Exception:
        pass
    return out

# ------------------------------------------------------------
# Säkerställ riktkurs-kolumner i Data-bladet
# ------------------------------------------------------------
RKT_COLS = ["Riktkurs idag","Riktkurs 1 år","Riktkurs 2 år","Riktkurs 3 år","Bull 1 år","Bear 1 år","Metod"]

def _ensure_riktkurs_cols(df: pd.DataFrame) -> pd.DataFrame:
    if df is None or df.empty:
        return pd.DataFrame(columns=[*DATA_COLUMNS, *RKT_COLS])
    out = df.copy()
    for c in RKT_COLS:
...
# ------------------------------------------------------------
# 🔍 Analys — enskilt bolag + fair value & metoder
# ------------------------------------------------------------
def page_analysis():
    st.header("🔍 Analys (enskilt bolag)")
    df: pd.DataFrame | None = st.session_state.get("DATA")
    if df is None or df.empty:
        df = read_data_df()
        st.session_state["DATA"] = df
    ...
    df = _ensure_riktkurs_cols(df)

    # Välj ticker med sök + bläddring
    names_map = _names_map_from_df(df)
    tickers = df["Ticker"].dropna().astype(str).unique().tolist()
    picked = _select_with_search_nav("Välj ticker", tickers, names_map, "analysis_idx", "analysis_q")
    if not picked:
        return

    row = df[df["Ticker"].astype(str).str.upper() == picked.upper()].head(1)
    if row.empty:
    ...
    try:
        try:
            res = compute_methods_for_row(row, settings=get_settings_map())
        except TypeError:
            res = compute_methods_for_row(row)
        methods_df = res.get("methods_df") if isinstance(res, dict) else None
        meta = res.get("meta") if isinstance(res, dict) else {}
        sanity = res.get("sanity") if isinstance(res, dict) else None
    except Exception as e:
        st.error(f"Beräkningsfel: {e}")
        return

    # Extrahera targets
    targets = _extract_targets_from_result(methods_df, meta)

    # Översikt
    c1, c2, c3, c4 = st.columns(4)
    with c1:
        st.metric("Aktuell kurs", f"{_f(row.get('Aktuell kurs')):.2f}" if _f(row.get("Aktuell kurs")) is not None else "—")
    with c2:
        st.metric("FV Idag", f"{targets['today']:.2f}" if _f(targets["today"]) is not None else "—")
    with c3:
        st.metric("FV 1 år", f"{targets['1y']:.2f}" if _f(targets["1y"]) is not None else "—")
    with c4:
        st.metric("FV 2 / 3 år",
                  (f"{targets['2y']:.2f} / {targets['3y']:.2f}")
                  if _f(targets["2y"]) is not None and _f(targets["3y"]) is not None else "—")

    if sanity:
        with st.expander("Sanity / Meta"):
            st.write(sanity)
            if isinstance(meta, dict) and meta:
                st.json(meta)

    st.subheader("Metod-tabell")
    if isinstance(methods_df, pd.DataFrame) and not methods_df.empty:
        # Visa med två decimaler vid behov
        show = methods_df.copy()
        for c in show.columns:
            if pd.api.types.is_numeric_dtype(show[c]):
                show[c] = show[c].map(lambda v: v if pd.isna(v) else float(v))
        _show_df(show, height=360, use_container_width=True)
    else:
        st.info("Inga metoder att visa.")

    # Uppdatera riktkurser i Data (frivilligt)
    st.markdown("---")
    st.subheader("Uppdatera riktkurser i Data för vald ticker")
    if st.button("💾 Skriv FV Idag/1/2/3 + Bull/Bear 1y till Data"):
        try:
            base = read_data_df()
            base = _ensure_riktkurs_cols(base)
            mask = base["Ticker"].astype(str).str.upper() == picked.upper()
            if not mask.any():
                st.error("Tickern finns inte i Data längre.")
                return
            idx = base.index[mask][0]
            def _set(k, v):
                if v is not None and _f(v) is not None:
                    base.at[idx, k] = round(float(_f(v)), 2)

            _set("Riktkurs idag", targets["today"])
            _set("Riktkurs 1 år", targets["1y"])
            _set("Riktkurs 2 år", targets["2y"])
            _set("Riktkurs 3 år", targets["3y"])
            _set("Bull 1 år", targets["bull_1y"])
            _set("Bear 1 år", targets["bear_1y"])
            base.at[idx, "Metod"] = "fair_value v2"
            write_data_df(base)
            st.session_state["DATA"] = base
            st.success(f"{picked}: riktkurser sparade.")
        except Exception as e:
            st.error(f"Kunde inte spara: {e}")

    # Visa hela databasen längst ner (krav)
    st.markdown("---")
    st.caption("Hela databasen (read-only)")
    try:
        all_df = st.session_state.get("DATA") or read_data_df()
        _show_df(all_df, height=320, use_container_width=True)
    except Exception:
        pass

# ------------------------------------------------------------
# 🏆 Ranking — massberäkna fair value & spara
# ------------------------------------------------------------
def _compute_targets_for_row(row: pd.Series) -> dict:
    try:
        try:
            res = compute_methods_for_row(row, settings=get_settings_map())
        except TypeError:
        ...
        methods_df = res.get("methods_df") if isinstance(res, dict) else None
        meta = res.get("meta") if isinstance(res, dict) else {}
        return _extract_targets_from_result(methods_df, meta)
    except Exception:
        return {"today": None, "1y": None, "2y": None, "3y": None, "bull_1y": None, "bear_1y": None}

def page_ranking():
    st.header("🏆 Ranking / Fair Value (massberäkning)")
    df = read_data_df()
    if df.empty:
        st.info("Data-bladet är tomt.")
        return
    ...
    tickers = sorted(df["Ticker"].dropna().astype(str).unique().tolist())
    sel = st.multiselect("Välj tickers (tom = alla)", options=tickers, default=[])
    target = [t for t in tickers if (not sel or t in sel)]

    # Kör
    go = st.button("🚀 Beräkna")
    if not go:
        return
    ...
    for i, t in enumerate(target, start=1):
        ...
        try:
            ...
            continue
        tvals = _compute_targets_for_row(row)

        # Skriv in i kopian (ej till Sheets än)
        mask = upd["Ticker"].astype(str).str.upper() == t.upper()
        if mask.any():
            idx = upd.index[mask][0]
            def _set(k, v):
                nonlocal changed
                if v is not None and _f(v) is not None:
                    nv = round(float(_f(v)), 2)
                    old = _f(upd.at[idx, k])
                    if (old is None) or (float(old) != nv):
                        upd.at[idx, k] = nv
                        changed += 1
            _set("Riktkurs idag", tvals["today"])
            _set("Riktkurs 1 år", tvals["1y"])
            _set("Riktkurs 2 år", tvals["2y"])
            _set("Riktkurs 3 år", tvals["3y"])
            _set("Bull 1 år", tvals["bull_1y"])
            _set("Bear 1 år", tvals["bear_1y"])
            upd.at[idx, "Metod"] = "fair_value v2"

            out_rows.append({
                "Ticker": t,
                "Kurs": _f(row.get("Aktuell kurs")),
                "FV Idag": _f(tvals["today"]),
                "FV 1 år": _f(tvals["1y"]),
                "FV 2 år": _f(tvals["2y"]),
                "FV 3 år": _f(tvals["3y"]),
                "Bull 1 år": _f(tvals["bull_1y"]),
                "Bear 1 år": _f(tvals["bear_1y"]),
            })
        progress.progress(i/len(target))
    ...
    if out_rows:
        ...
        # sortera på uppsida idag om kurs finns
        try:
            res["Uppsida idag (%)"] = np.where(
                (pd.to_numeric(res["Kurs"], errors="coerce") > 0) & (pd.to_numeric(res["FV Idag"], errors="coerce") > 0),
                (pd.to_numeric(res["FV Idag"], errors="coerce") - pd.to_numeric(res["Kurs"], errors="coerce"))
                / pd.to_numeric(res["Kurs"], errors="coerce") * 100.0,
                np.nan
            )
        except Exception:
            pass
        show = res.copy()
        for c in ["Kurs","FV Idag","FV 1 år","FV 2 år","FV 3 år","Bull 1 år","Bear 1 år"]:
            if c in show.columns:
                show[c] = show[c].map(lambda v: "" if _f(v) is None else f"{float(v):.2f}")
        if "Uppsida idag (%)" in show.columns:
            show["Uppsida idag (%)"] = show["Uppsida idag (%)"].map(
                lambda v: "" if _f(v) is None else f"{float(v):.1f}%"
            )
        # Sortering: störst uppsida först
        if "Uppsida idag (%)" in res.columns:
            res_sorted = res.sort_values("Uppsida idag (%)", ascending=False)
            show = show.loc[res_sorted.index]
        st.subheader("Resultat (ej sparat ännu)")
        _show_df(show.reset_index(drop=True), height=420, use_container_width=True)
    else:
        st.info("Inget att visa.")
        return

    # Spara till Google Sheets
    if st.button(f"💾 Spara {changed} ändringar till Data"):
        try:
            write_data_df(upd)
            st.session_state["DATA"] = upd
            st.success("Riktkurser sparade.")
        except Exception as e:
            st.error(f"Kunde inte spara: {e}")
...
def page_home():
    ...
    st.caption("Basversion 2025-11-16 — endast buggfixar och småförbättringar.")
    try:
        ...
        st.caption("Snabbvy: 5 minsta innehav (SEK)")
        if not pos.empty:
            small = pos.sort_values("Värde (SEK)").head(5)
            _show_df(small, height=220, use_container_width=True)
    except Exception:
        pass
    st.markdown("---")
    st.write("**Snabbnavigation**")
    st.write("• 🔍 Analys — enskilt bolag med fair value-metoder")
    st.write("• 🏆 Ranking — massberäkna riktkurser (Idag/1/2/3) och spara")
    st.write("• 📦 Portfölj — innehav och kommande utdelningar")
    st.write("• ✏️ Editor / ➕ Lägg till — manuella ändringar & nya tickers")
    st.write("• 🧩 Massuppdatering — hämta fält från Yahoo (1s per bolag)")
    st.write("• 🕒 Snapshot — historik")
    st.write("• ⚙️ Settings — inställningar & källskatt")
...
def _bootstrap_session():
    # Ladda till session (första gången)
...
def main():
    ...
    with st.sidebar:
        st.title("📊 Navigering")
        choice = st.selectbox("Välj sida", list(PAGES.keys()), index=0)
        st.markdown("---")
        st.caption("All data visas i bolagets valuta (ingen FX på EPS).")
        st.caption("Riktkurser skrivs med två decimaler.")
    # Kör vald sida
    try:
        PAGES[choice]()
    except Exception as e:
