...
#  • ✅ Nya förslag: Bucket-tak i Settings → köp-/trim-förslag
#     – Köp kräver: Fair Value (idag) > aktuell kurs
#     – Hela antal aktier, priser i handelsvaluta men beslut på SEK-bas
# ============================================================
...
def _build_portfolio_table(data_df: pd.DataFrame, fx_map: dict[str, float], settings: dict[str, str]) -> pd.DataFrame:
    ...
    Returnerar en per-innehav-tabell med:
      Ticker, Bolagsnamn, Bucket, Valuta, Antal, Kurs, Värde (SEK), GAV (SEK),
      Kostnad (SEK), P/L (SEK, %), Årlig utd/aktie (brutto/netto),
      Årlig utd (valuta/SEK), Yield (netto)
    """
    cols_out = [
    ...
    for _, r in data_df.iterrows():
        ...
        ccy    = str(r.get("Valuta") or "USD").upper().strip()

        price  = _pos(r.get("Aktuell kurs"))
        gav    = _f(r.get("GAV (SEK)"))  # per aktie i SEK
        dps_br = _pos(r.get("Årlig utdelning"))  # års-takt per aktie (brutto) i handelsvalutan
...
# -------------------------
# Nya: Bucket-tak (Settings) + förslagsmotor
# -------------------------
def _bucket_cap_map_from_settings(settings: dict[str, str]) -> dict[str, Optional[float]]:
    """
    Läser Bucket-tak (per INNEHAV) i SEK från Settings.
    Keys (valfria, skrivs utan å/ä/ö):
    ...
    Mappar mot DEFAULT_BUCKETS.
    """
    def _cap(key: str) -> Optional[float]:
        v = settings.get(key)
        fv = _f(v)
        return float(fv) if fv is not None and fv > 0 else None

    mp = {
        "Bucket A tillväxt": _cap("bucket_cap_A_tillvaxt"),
        "Bucket B tillväxt": _cap("bucket_cap_B_tillvaxt"),
        "Bucket C tillväxt": _cap("bucket_cap_C_tillvaxt"),
        "Bucket A utdelning": _cap("bucket_cap_A_utdelning"),
        "Bucket B utdelning": _cap("bucket_cap_B_utdelning"),
        "Bucket C utdelning": _cap("bucket_cap_C_utdelning"),
    }
    return mp

def _build_fair_value_today_map(data_df: pd.DataFrame, settings: dict[str, str], fx_map: dict[str, float]) -> dict[str, Optional[float]]:
    """
    Bygger {ticker: fair_value_today} i handelsvalutan.
    Försöker först använda sparad 'Riktkurs idag' som proxy.
    Om saknas → beräkna via compute_methods_for_row (median över familjer).
    """
    fv: dict[str, Optional[float]] = {}
    # Försök från sparat
    if "Ticker" in data_df.columns and "Riktkurs idag" in data_df.columns:
        for _, r in data_df[["Ticker","Riktkurs idag"]].iterrows():
            t = str(r.get("Ticker") or "").strip()
            if not t:
                continue
            val = _f(r.get("Riktkurs idag"))
            fv[t] = float(val) if val is not None else None

    # Beräkna där None
    tickers = [t for t in data_df["Ticker"].astype(str).unique().tolist() if t.strip()]
    for t in tickers:
        if fv.get(t) is not None:
            continue
        # Hämta rad
        row = data_df.loc[data_df["Ticker"].astype(str) == t]
        if row.empty:
            fv[t] = None
            continue
        try:
            methods_df, sanity, meta = compute_methods_for_row(row.iloc[0], settings, fx_map)
            fv_today = _f((meta or {}).get("fair_value", {}).get("today"))
            fv[t] = float(fv_today) if fv_today is not None else None
        except Exception:
            fv[t] = None
    return fv

def _compute_bucket_suggestions(data_df: pd.DataFrame,
                                pf_df: pd.DataFrame,
                                fx_map: dict[str, float],
                                settings: dict[str, str]) -> tuple[pd.DataFrame, pd.DataFrame]:
    """
    Returnerar (trim_df, buy_df).
    • Trim: om 'Värde (SEK)' > cap → föreslå antal att sälja = floor((diff_sek) / pris_sek)
    • Köp: om 'Värde (SEK)' < cap OCH Fair Value idag > Kurs → antal att köpa (heltal)
    """
    if pf_df.empty:
        return (pd.DataFrame(), pd.DataFrame())

    bucket_caps = _bucket_cap_map_from_settings(settings)
    fv_map = _build_fair_value_today_map(data_df, settings, fx_map)

    trims = []
    buys  = []

    # Snabbmap för radinfo (pris & valuta per ticker) från DATA
    data_price = {}
    data_ccy   = {}
    for _, r in data_df.iterrows():
        t = str(r.get("Ticker") or "").strip()
        if not t:
            continue
        data_price[t] = _f(r.get("Aktuell kurs"))
        data_ccy[t]   = str(r.get("Valuta") or "USD").upper().strip()

    for _, row in pf_df.iterrows():
        tkr    = str(row.get("Ticker") or "").strip()
        bucket = str(row.get("Bucket") or "").strip()
        cur_val_sek = _f(row.get("Värde (SEK)"))
        shares = _pos(row.get("Antal aktier")) or 0.0
        name   = str(row.get("Bolagsnamn") or "")
        if not tkr or not bucket or cur_val_sek is None or shares <= 0:
            continue

        cap_sek = bucket_caps.get(bucket)
        if cap_sek is None or cap_sek <= 0:
            # inget tak angivet → hoppa över
            continue

        # Pris i handelsvaluta + SEK
        px  = _pos(data_price.get(tkr))
        ccy = data_ccy.get(tkr) or "USD"
        px_sek = _fx_to_sek(px, ccy, fx_map) if _pos(px) else None
        if px_sek is None or px_sek <= 0:
            continue

        # ---- TRIM ----
        if cur_val_sek > cap_sek:
            diff = cur_val_sek - cap_sek
            n_to_sell = math.floor(diff / px_sek) if diff > 0 else 0
            n_to_sell = min(n_to_sell, int(shares))  # kan inte sälja fler än man äger
            if n_to_sell >= 1:
                trims.append({
                    "Bucket": bucket,
                    "Ticker": tkr,
                    "Bolagsnamn": name,
                    "Kurs": round(float(px), 4),
                    "Valuta": ccy,
                    "Kurs (SEK)": round(float(px_sek), 2),
                    "Värde nu (SEK)": round(float(cur_val_sek), 0),
                    "Tak (SEK)": round(float(cap_sek), 0),
                    "Diff (SEK)": round(float(diff), 0),
                    "Föreslagen trim (antal)": int(n_to_sell),
                })

        # ---- KÖP ----
        elif cur_val_sek < cap_sek:
            # Köp endast om FV idag > pris
            fv = _pos(fv_map.get(tkr))
            if fv is None or px is None:
                continue
            if fv <= px:
                continue  # inte undervärderad

            add = cap_sek - cur_val_sek
            n_to_buy = math.floor(add / px_sek) if add > 0 else 0
            if n_to_buy >= 1:
                upside_pct = ((fv - px) / px) * 100.0 if px else None
                buys.append({
                    "Bucket": bucket,
                    "Ticker": tkr,
                    "Bolagsnamn": name,
                    ...
                    "Valuta": ccy,
                    "Kurs (SEK)": round(float(px_sek), 2),
                    "Fair Value (idag)": round(float(fv), 4),
                    "Uppsida vs FV (%)": None if upside_pct is None else round(float(upside_pct), 1),
                    "Värde nu (SEK)": round(float(cur_val_sek), 0),
                    "Tak (SEK)": round(float(cap_sek), 0),
                    "Diff till tak (SEK)": round(float(add), 0),
                    "Föreslaget köp (antal)": int(n_to_buy),
                })

    trim_df = pd.DataFrame(trims) if trims else pd.DataFrame(columns=[
        "Bucket","Ticker","Bolagsnamn","Kurs","Valuta","Kurs (SEK)","Värde nu (SEK)",
        "Tak (SEK)","Diff (SEK)","Föreslagen trim (antal)"
    ])
    buy_df = pd.DataFrame(buys) if buys else pd.DataFrame(columns=[
        "Bucket","Ticker","Bolagsnamn","Kurs","Valuta","Kurs (SEK)","Fair Value (idag)",
        "Uppsida vs FV (%)","Värde nu (SEK)","Tak (SEK)","Diff till tak (SEK)","Föreslaget köp (antal)"
    ])

    if not trim_df.empty:
        trim_df = trim_df.sort_values(["Bucket","Diff (SEK)"], ascending=[True, False], na_position="last").reset_index(drop=True)
    if not buy_df.empty:
        # sortera mest uppsida först, sedan störst gap till tak
        buy_df = buy_df.sort_values(
            ["Uppsida vs FV (%)","Diff till tak (SEK)"],
            ascending=[False, False],
            na_position="last"
        ).reset_index(drop=True)

    return trim_df, buy_df

# -------------------------
# UI: Portföljvy (förenklad + expanders per bucket + förslag)
# -------------------------
def render_portfolio_view(data_df: pd.DataFrame, fx_map: dict[str, float]) -> None:
    ...
    # ---------- ✅ Förslag: Trim & Köp baserat på Bucket-tak + FV-villkor ----------
    st.markdown("### Förslag (Bucket-tak)")
    caps = _bucket_cap_map_from_settings(settings)
    any_caps = any(v is not None and v > 0 for v in caps.values())
    if not any_caps:
        st.info("Inga Bucket-tak definierade ännu. Lägg till i Settings (t.ex. bucket_cap_A_tillvaxt = 20000).")
    else:
        with st.spinner("Beräknar förslag…"):
            trim_df, buy_df = _compute_bucket_suggestions(data_df, pf, fx_map, settings)

        c1, c2 = st.columns(2)
        with c1:
            st.subheader("✂️ Trimförslag")
            if trim_df.empty:
                st.caption("Inga trimförslag baserat på nuvarande tak.")
            else:
                st.dataframe(trim_df, use_container_width=True, hide_index=True)
        with c2:
            st.subheader("🧩 Köpförslag (FV>pris)")
            if buy_df.empty:
                st.caption("Inga köpförslag som både är under tak och undervärderade.")
            else:
...
# ============================================================
# Del 5/6 — Vyer
#  • Settings & Valutakurser  ← ✅ Bucket-tak (per innehav, SEK)
#  • Snapshot
...
# ============================================================
# Settings-vy + valutakurser  ← ✅ Bucket-tak (per innehav, SEK)
# ============================================================
def page_settings():
...
#  • Init/State (DATA, FX, Settings)
#  • Sidopanel & navigering
#  • Säkert sidbyte + felhantering
# ============================================================

# ---------- Init & state ----------
def _load_state_if_needed(force_reload: bool = False):
    """Säkerställ att DATA, FX och SETTINGS finns i sessionen."""
    if force_reload or ("DATA" not in st.session_state):
        try:
            ...
            st.sidebar.error(f"Kunde inte läsa DATA från Google Sheets: {e}")
    ...
    if force_reload or ("SETTINGS" not in st.session_state):
        try:
...
def _maybe_auto_refresh_fx():
    """Uppdatera valutakurser vid start om inställt i Settings."""
    try:
        ...
        if auto and not st.session_state.get("_fx_autorefreshed_once"):
            ...
            st.toast("Valutakurser uppdaterade automatiskt.")
    except Exception as e:
        st.sidebar.warning(f"Auto FX-uppdatering misslyckades: {e}")
...
def _render_sidebar() -> str:
    ...
    s = st.session_state.get("SETTINGS", {}) or {}
    fx = st.session_state.get("FX", {}) or {}
    base_ccy = s.get("primary_currency", "SEK")
    st.sidebar.caption(f"Primär valuta: **{base_ccy}**")
    st.sidebar.caption(f"Valutakurser: {len(fx)} st")

    # Actions
    col_a, col_b = st.sidebar.columns(2)
    with col_a:
        if st.button("↻ Ladda om DATA"):
            _load_state_if_needed(force_reload=True)
            st.rerun()
    with col_b:
    ...
    st.session_state["_PAGES_MAP"] = pages  # spara för main()

    page_names = list(pages.keys())
    default_page = st.session_state.get("_current_page", page_names[0])
    sel = st.sidebar.radio("Välj vy", page_names, index=page_names.index(default_page) if default_page in page_names else 0)
    st.session_state["_current_page"] = sel

    st.sidebar.markdown("---")
    if st.sidebar.checkbox("Visa rådata (session)"):
        st.sidebar.dataframe(st.session_state.get("DATA"), use_container_width=True)
...
def main():
    try:
        _load_state_if_needed(force_reload=False)
        _maybe_auto_refresh_fx()
        sel = _render_sidebar()

        pages = st.session_state.get("_PAGES_MAP", {})
        view_fn = pages.get(sel)
        if not view_fn:
            st.error("Okänd vy.")
            return

        # Kör vald vy med felhantering så resten av appen lever vidare vid fel
        try:
            view_fn()
        except Exception as e:
            st.error(f"💥 Fel i vyn **{sel}**: {e}")
            st.exception(e)
