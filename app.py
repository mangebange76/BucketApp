...
def _with_backoff(callable_fn, *args, **kwargs):
    """Kör gspread-anrop med enkel exponential backoff vid 429."""
    delay = 0.6
    for attempt in range(6):
        try:
            return callable_fn(*args, **kwargs)
        except APIError as e:
            # 429 eller tillfälligt fel → backoff
...
def _open_spreadsheet(_gc: gspread.Client) -> Spreadsheet:
    """Öppnar spreadsheet via SHEET_URL eller SHEET_ID."""
...
def _get_ws(spread: Spreadsheet, title: str) -> Worksheet:
    """Hitta eller skapa blad TITEL på ett säkert sätt (utan egen Worksheet-init)."""
    try:
...
def compute_methods_for_row(row: pd.Series, settings: Dict[str, str], fx_map: Dict[str, float]) -> Tuple[pd.DataFrame, str, Dict[str, Any]]:
    ...
    methods.append({"Metod": "ev_sales",        "Idag": _equity_price_from_ev(_ev_from_sales(r0, evs0), net_debt, shares),
    ...
    methods.append({"Metod": "ev_ebitda",       "Idag": _equity_price_from_ev(_ev_from_ebitda(b0, eve0), net_debt, shares),
    ...
    methods.append({"Metod": "ev_dacf",         "Idag": _equity_price_from_ev(_ev_from_ebitda(b0, eve0), net_debt, shares),
                                         "1 år": _equity_price_from_ev(_ev_from_ebitda(b1, eve1), net_debt, shares),
                                         "2 år": _equity_price_from_ev(_ev_from_ebitda(b2, eve2), net_debt, shares),
                                         "3 år": _equity_price_from_ev(_ev_from_ebitda(b3, eve3), net_debt, shares)})
...
def _fmt_money(v: Optional[float], ccy: str) -> str:
    if v is None: return "–"
    try: return f"{v:,.2f} {ccy}".replace(",", " ").replace(".", ",")
    except Exception: return f"{v} {ccy}"

def _fmt_sek(v: Optional[float]) -> str:
    if v is None: return "0 SEK"
    try: return f"{v:,.0f} SEK".replace(",", " ").replace(".", ",")
    except Exception: return f"{v} SEK"
...
def get_withholding_for(currency: str, settings: Dict[str, str]) -> float:
    ...
    try: return float(settings.get(key, "0.15"))
    except Exception: return 0.15
...
# ---------- Primärmetod-val (respektera fältet) ----------
def _choose_primary(met_df: pd.DataFrame, override: Optional[str]) -> Tuple[Optional[str], Optional[float], Optional[float], Optional[float], Optional[float]]:
    if met_df is None or met_df.empty:
        return None, None, None, None, None
    met_df = met_df.copy()

    # 1) Om användaren har valt "Primär metod" i Data – försök använda den
    if override:
        override = str(override).strip()
        if override in met_df["Metod"].values:
            row = met_df[met_df["Metod"] == override].iloc[0]
            if _pos(row["Idag"]):
                return override, _f(row["Idag"]), _f(row["1 år"]), _f(row["2 år"]), _f(row["3 år"])

    # 2) Annars: heuristik – flest giltiga värden, sedan preferenslista
...
def render_company_view(row: pd.Series, settings: Dict[str, str], fx_map: Dict[str, float]) -> None:
    ...
    # Primär riktkurs – RESPEKTERA fältet "Primär metod"
    primary, p0, p1, p2, p3 = _choose_primary(met_df, _nz(row.get("Primär metod"), None))
    st.markdown(f"#### 🎯 Primär riktkurs ({primary or '—'})")
    cols = st.columns(4)
    ...
    st.caption(f"Valuta: **{currency}** • PE-ankare vikt (TTM): {int(float(settings.get('pe_anchor_weight_ttm','0.5'))*100)}% • Multipel-decay/år: {settings.get('multiple_decay','0.10')}")
    ...
    if c1.button("🔄 Uppdatera Estimat/CAGR", key=f"upd_est_{tkr}"):
        snap = fetch_yahoo_snapshot(tkr); est = fetch_finnhub_estimates(tkr)
        eps_ttm = _pos(_nz(snap.get("eps_ttm"), row.get("EPS TTM"))); eps1 = _pos(est.get("eps_1y"))
        eps_cagr = _f(row.get("EPS CAGR"))
        if eps_cagr is None and _pos(eps_ttm) and _pos(eps1):
            try: eps_cagr = (eps1/eps_ttm) - 1.0
            except Exception: eps_cagr = None
        rev_cagr = _f(row.get("Rev CAGR"))
    ...
    if c2.button("💾 Spara primär riktkurs till Resultat", key=f"save_res_{tkr}"):
        append_result_row({
            ...
            "Valuta": currency,
            "Metod": primary or "",
            "Riktkurs idag": p0, "Riktkurs 1 år": p1, "Riktkurs 2 år": p2, "Riktkurs 3 år": p3,
        })
...
def save_quarter_snapshot(ticker: str, methods_df: pd.DataFrame, meta: Dict[str, Any]) -> None:
    ...
    for _, r in methods_df.iterrows():
        rows.append({
            "Timestamp": ts, "Ticker": ticker, "Valuta": meta.get("currency"),
            "Metod": r.get("Metod"), "Idag": _f(r.get("Idag")), "1 år": _f(r.get("1 år")),
            "2 år": _f(r.get("2 år")), "3 år": _f(r.get("3 år")),
            "Ankare PE": _f(meta.get("pe_anchor")), "Decay": _f(meta.get("decay")),
        })
...
# ============================================================
# SIDA: Editor – fixad sparning (kolumn-union + cache-clear)
# ============================================================
def page_editor():
    ...
    tickers = ["— nytt —"] + sorted(df["Ticker"].dropna().astype(str).unique().tolist()) if not df.empty else ["— nytt —"]
    ...
    if fetch_btn and ticker:
        snap = fetch_yahoo_snapshot(ticker)
        st.info(f"Hämtat: pris={snap.get('price')} {snap.get('currency')}, MCAP={snap.get('market_cap')}, EV/Rev={snap.get('ev_to_sales')}, EV/EBITDA={snap.get('ev_to_ebitda')}, P/B={snap.get('p_to_book')}, BVPS={snap.get('bvps')}, EPS TTM={snap.get('eps_ttm')}")
        st.caption("Fyll i rutan ovan med värden du vill spara, klicka sedan **Spara**.")

    if save_btn and ticker:
        df_old = read_data_df()
        ts = now_stamp()
        new_row = {
            "Timestamp": ts, "Ticker": ticker, "Bolagsnamn": name, "Sektor": sector,
            "Bucket": bucket, "Valuta": valuta, "Antal aktier": antal, "GAV (SEK)": gav_sek,
            "Aktuell kurs": kurs, "Utestående aktier": shares, "Net debt": net_debt,
            "Rev TTM": rev_ttm, "EBITDA TTM": ebitda_t, "EPS TTM": eps_ttm,
        ...
        # Kolumn-union
        cols = list(df_old.columns) if not df_old.empty else list(DATA_COLUMNS)
        ...
        if df_old.empty:
            ...
            if mask.any():
                ...
                for k in cols:
                    if k not in add_row.columns:
                        add_row[k] = np.nan
                add_row = add_row[cols]
                df_new = pd.concat([df_new, add_row], ignore_index=True)
...
def page_ranking():
    ...
    settings = get_settings_map(); fx_map = get_fx_map(); df = read_data_df()
    if df.empty:
        st.info("Data-bladet är tomt."); return
    ...
    for i, (_, r) in enumerate(q.iterrows()):
        try:
            ...
            method, p0, _, _, _ = _choose_primary(met_df, _nz(r.get("Primär metod"), None))
            price = meta.get("price")
            currency = meta.get("currency") or str(_nz(r.get("Valuta"), "USD")).upper()
            upside = None
            if _pos(price) and _pos(p0):
                upside = (p0/price - 1.0) * 100.0
            rows.append({"Ticker": r.get("Ticker"), "Bolagsnamn": r.get("Bolagsnamn"), "Bucket": r.get("Bucket"),
                         "Valuta": currency, "Pris": price, "Primär metod": method, "Fair value (Idag)": p0, "Uppsida %": upside})
            time.sleep(0.2)
        except Exception:
            rows.append({"Ticker": r.get("Ticker"), "Bolagsnamn": r.get("Bolagsnamn"), "Bucket": r.get("Bucket"),
                         "Valuta": r.get("Valuta"), "Pris": None, "Primär metod": None, "Fair value (Idag)": None, "Uppsida %": None})
...
# ============================================================
# SIDA: Inställningar & Batch
# ============================================================
def page_settings():
    st.header("⚙️ Inställningar")
    settings = get_settings_map(); fx_map = get_fx_map()
    ...
    if w_submit:
        ...
        def upsert(k, v, s=s):
            if (s["Key"] == k).any(): s.loc[s["Key"] == k, "Value"] = str(v)
            else: s = pd.concat([s, pd.DataFrame([[k, str(v)]], columns=SETTINGS_COLUMNS)], ignore_index=True)
            return s
...
def page_batch():
    ...
    fx_map   = get_fx_map()
    if df.empty:
        st.info("Inga bolag i Data."); return
    ...
    if st.button("🔄 Uppdatera pris & nycklar från Yahoo (alla)"):
        prog = st.progress(0.0); df2 = df.copy()
        for i, (idx, r) in enumerate(df2.iterrows()):
            ...
            time.sleep(throttle); prog.progress((i+1)/len(df2))
        write_data_df(df2); prog.empty(); st.success("Uppdaterat alla tickers från Yahoo.")

    if st.button("📷 Spara snapshots (alla)"):
        prog = st.progress(0.0); count = 0
        for i, (_, r) in enumerate(df.iterrows()):
            met_df, _, meta = compute_methods_for_row(r, settings, fx_map)
            save_quarter_snapshot(str(r["Ticker"]).strip().upper(), met_df, meta)
            count += 1; time.sleep(throttle); prog.progress((i+1)/len(df))
        prog.empty(); st.success(f"Snapshot sparade för {count} bolag.")
...
def run_main_ui():
    ...
    with st.expander("📊 Status (FX & inställningar)", expanded=False):
        fx_map = get_fx_map(); st.write("FX (SEK per 1):", fx_map)
        settings = get_settings_map(); st.write("Settings:", settings)

    page = st.sidebar.radio("Sidor", ["Editor", "Analys", "Ranking", "Inställningar", "Batch"], index=1)
    if page == "Editor":        page_editor()
    elif page == "Analys":      page_analysis()
    elif page == "Ranking":     page_ranking()
    elif page == "Inställningar": page_settings()
