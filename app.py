...
# ============================================================
# Del 4/6 — Analys & Ranking-vy
#  • Vy: Analys (enskilt bolag + metodtabell + sanity)
#  • Vy: Ranking (sortering på FV: Idag / 1 / 2 / 3 år, uppsida %)
#  • Bläddringsfunktion (1/X) för sorterad lista
#  • Alltid: hela databasen längst ned som enkel tabell (ofiltererad)
# ============================================================

# -------------------------
# Små UI-hjälpare (ej valutaomräkning)
# -------------------------
def _fmt2(x: Optional[float]) -> str:
    v = _f(x)
    return "" if v is None or (isinstance(v, float) and (not math.isfinite(v))) else f"{float(v):.2f}"

def _fmt_money(x: Optional[float], ccy: str) -> str:
    s = _fmt2(x)
    return s if not s else f"{s} {ccy}"

def _upside_pct(price: Optional[float], target: Optional[float]) -> Optional[float]:
...
# -------------------------
# Enskilt bolag – analyspanel
# -------------------------
def _panel_single_analysis(row: pd.Series, settings: Dict[str, str], fx_map: Dict[str, float]):
    methods_df, sanity, meta = compute_methods_for_row(row, settings, fx_map)
    ccy   = (meta.get("currency") or "USD")
    price = meta.get("price")
    fv    = meta.get("fair_value") or {}
    st.subheader(f"🧩 Analys: {row.get('Ticker')}")

    colA, colB, colC, colD, colE = st.columns([1,1,1,1,2])
    with colA:
        st.metric("Aktuell kurs", _fmt_money(price, ccy))
    with colB:
        st.metric("Fair Value idag", _fmt_money(fv.get("today"), ccy), delta=f"{_fmt2(_upside_pct(price, fv.get('today')))} %")
    with colC:
        st.metric("FV om 1 år", _fmt_money(fv.get("y1"), ccy), delta=f"{_fmt2(_upside_pct(price, fv.get('y1')))} %")
    with colD:
        st.metric("FV om 2 år", _fmt_money(fv.get("y2"), ccy), delta=f"{_fmt2(_upside_pct(price, fv.get('y2')))} %")
    with colE:
        st.metric("FV om 3 år", _fmt_money(fv.get("y3"), ccy), delta=f"{_fmt2(_upside_pct(price, fv.get('y3')))} %")

    st.caption(sanity)

    # Metodtabell (alla priser i bolagets handelsvaluta)
    tbl = methods_df.copy()
    for col in ["Idag", "1 år", "2 år", "3 år"]:
        tbl[col] = tbl[col].apply(lambda v: _fmt_money(v, ccy))
    st.dataframe(tbl, use_container_width=True, hide_index=True)

# -------------------------
# Ranking – sortering & bläddring
# -------------------------
def _build_ranking(df: pd.DataFrame, settings: Dict[str, str], fx_map: Dict[str, float]) -> pd.DataFrame:
    rows = []
    for _, r in df.iterrows():
        try:
            ...
            rows.append({
                ...
                "price": None, "currency": r.get("Valuta") or "",
                "fv_today": None, "fv_1y": None, "fv_2y": None, "fv_3y": None,
                "sanity": f"error: {e}",
            })
    ranking = pd.DataFrame(rows)
    # Uppsidor
    ranking["Upside % (Idag)"] = ranking.apply(lambda x: _upside_pct(x.get("price"), x.get("fv_today")), axis=1)
    ranking["Upside % (1 år)"] = ranking.apply(lambda x: _upside_pct(x.get("price"), x.get("fv_1y")), axis=1)
    ranking["Upside % (2 år)"] = ranking.apply(lambda x: _upside_pct(x.get("price"), x.get("fv_2y")), axis=1)
    ranking["Upside % (3 år)"] = ranking.apply(lambda x: _upside_pct(x.get("price"), x.get("fv_3y")), axis=1)
    return ranking

def _panel_ranking(df: pd.DataFrame, settings: Dict[str, str], fx_map: Dict[str, float]):
    st.subheader("🏆 Ranking (Fair Value & uppsida)")
    ranking = _build_ranking(df, settings, fx_map)
    ...
    with col1:
        horizon = st.selectbox("Sortera på riktkurs", list(sort_key_map.keys()), index=0)
    with col2:
        descending = st.checkbox("Störst uppsida först", value=True)

    key = sort_key_map[horizon]
    ranking_sorted = ranking.sort_values(by=key, ascending=not descending, na_position="last").reset_index(drop=True)

    # Bläddringsfunktion (1/X)
    n = len(ranking_sorted)
    if "rank_idx" not in st.session_state:
        st.session_state.rank_idx = 0

    colA, colB, colC = st.columns([1,2,6])
    with colA:
        if st.button("◀ Föregående", use_container_width=True):
            st.session_state.rank_idx = (st.session_state.rank_idx - 1) % max(n, 1)
    with colB:
        st.write(f"Post **{st.session_state.rank_idx+1} / {n}**")
    with colC:
        if st.button("Nästa ▶", use_container_width=True):
            st.session_state.rank_idx = (st.session_state.rank_idx + 1) % max(n, 1)

    # Visa vald rad i en kompakt karta
    if n > 0:
        r = ranking_sorted.iloc[st.session_state.rank_idx]
        ccy = r.get("currency") or ""
        box = {
            "Ticker": r.get("ticker"),
            "Kurs": _fmt_money(r.get("price"), ccy),
            "FV Idag": _fmt_money(r.get("fv_today"), ccy),
            "FV 1 år": _fmt_money(r.get("fv_1y"), ccy),
            "FV 2 år": _fmt_money(r.get("fv_2y"), ccy),
            "FV 3 år": _fmt_money(r.get("fv_3y"), ccy),
            "Upside (Idag)": f"{_fmt2(r.get('Upside % (Idag)'))} %",
            "Upside (1 år)": f"{_fmt2(r.get('Upside % (1 år)'))} %",
            "Upside (2 år)": f"{_fmt2(r.get('Upside % (2 år)'))} %",
            "Upside (3 år)": f"{_fmt2(r.get('Upside % (3 år)'))} %",
        }
        st.info("**Vald post (sorterad lista):**\n\n" + "\n".join([f"- **{k}:** {v}" for k, v in box.items()]))

    # Visningstabell
    vis = ranking_sorted[[
        "ticker", "currency", "price", "fv_today", "fv_1y", "fv_2y", "fv_3y",
        "Upside % (Idag)", "Upside % (1 år)", "Upside % (2 år)", "Upside % (3 år)"
    ]].rename(columns={
        "ticker":"Ticker","currency":"Valuta","price":"Kurs",
        "fv_today":"FV Idag","fv_1y":"FV 1 år","fv_2y":"FV 2 år","fv_3y":"FV 3 år"
    }).copy()

    # Formatera för läsbarhet
    for c in ["Kurs","FV Idag","FV 1 år","FV 2 år","FV 3 år"]:
        # Lägg till valuta per rad
        vis[c] = [ _fmt_money(vis.loc[i, c], vis.loc[i, "Valuta"]) for i in vis.index ]
    for c in ["Upside % (Idag)","Upside % (1 år)","Upside % (2 år)","Upside % (3 år)"]:
        vis[c] = vis[c].apply(lambda v: "" if v is None else f"{float(v):.2f} %")

    st.dataframe(vis, use_container_width=True, hide_index=True)

# -------------------------
# Publik vy-funktion (anropas från main)
# -------------------------
def view_analysis_and_ranking(data_df: pd.DataFrame, settings: Dict[str, str], fx_map: Dict[str, float]):
    st.header("🔎 Analys & Ranking")

    # Välj enskilt bolag
    tickers = [str(t).upper() for t in data_df.get("Ticker", pd.Series(dtype=str)).fillna("").tolist() if str(t).strip()!=""]
    sel = st.selectbox("Välj bolag för detaljerad analys", ["(välj)"] + tickers, index=0)
    if sel and sel != "(välj)":
        row = data_df[data_df["Ticker"].astype(str).str.upper()==sel].iloc[0]
        _panel_single_analysis(row, settings, fx_map)

    st.divider()
    _panel_ranking(data_df, settings, fx_map)

    st.divider()
    st.subheader("📋 Hela databasen (ofiltererad, enkel tabell)")
    st.caption("Visar alltid alla kolumner exakt som i databasen.")
...
def page_analysis():
    ...
    # === Alltid visa hela databasen längst ned (ofiltererad, enkel) ===
    st.markdown("---")
    st.subheader("📋 Hela databasen (ofiltererad, enkel tabell)")
...
#  • Sidomeny & routning
#  • Felhantering i vy-anrop
# ============================================================

# -----------------------------
# Init / session helpers
# -----------------------------
def _session_get(name: str, fallback=None):
    return st.session_state.get(name, fallback)

def _load_all_into_session(force: bool = False) -> None:
    """
    Laddar DATA, FX och Settings in i session. Använder befintliga helpers
    från tidigare delar: read_data_df(), get_fx_map(), get_settings_map().
    """
    if force or _session_get("DATA") is None:
        try:
            st.session_state["DATA"] = read_data_df()
        except Exception as e:
            st.session_state["DATA"] = pd.DataFrame(columns=DATA_COLUMNS)
            st.warning(f"Kunde inte läsa DATA från Sheets: {e}")

    if force or _session_get("FX") is None:
        try:
    ...
    if force or _session_get("SETTINGS_MAP") is None:
        try:
            st.session_state["SETTINGS_MAP"] = get_settings_map()
        except Exception as e:
...
def _router_pages() -> dict[str, dict]:
    """
    Mappar menyval → render-funktion. Håll ordningen konsekvent.
    """
    return {
        "🔬 Analys":          {"fn": page_analysis},
        "🏆 Ranking":         {"fn": page_ranking},
        "📦 Portfölj":        {"fn": page_portfolio},
        "🛒 Köpförslag":      {"fn": page_buy_suggestions},
        "✏️ Editor":          {"fn": page_editor},
        "➕ Lägg till":       {"fn": page_add_ticker},
        "🧩 Batch":           {"fn": page_batch},
        "🕒 Snapshot":        {"fn": page_snapshot},
        "⚙️ Settings":        {"fn": page_settings},
    }

def _set_query_page(label: str) -> None:
    try:
        st.query_params.update(page=label)
    except Exception:
        pass

def _get_query_page(default_label: str) -> str:
    try:
        qp = st.query_params
        if "page" in qp and qp["page"] in _router_pages().keys():
            return qp["page"]
    except Exception:
        pass
...
def _render_sidebar():
    pages = _router_pages()
    default_page = "🔬 Analys"
    current = _get_query_page(default_page)

    with st.sidebar:
        st.markdown("## 📈 Aktieanalys & investeringsförslag")
        st.caption("Basversion låst • Alla funktioner bevarade")

        # Snabbstatus
        try:
            df = _session_get("DATA", pd.DataFrame())
            nrows = len(df) if isinstance(df, pd.DataFrame) else 0
            st.metric("Rader i DATA", nrows)
        except Exception:
            st.metric("Rader i DATA", 0)

        # Kontrollknappar
        c1, c2 = st.columns(2)
        with c1:
            if st.button("🔄 Uppdatera", help="Läs om DATA/FX/Settings från Google Sheets"):
                _load_all_into_session(force=True)
                st.experimental_rerun()  # säker uppdatering
        with c2:
            if st.button("🧹 Töm cache", help="Töm Streamlits cache"):
                try:
                    st.cache_data.clear()
                except Exception:
                    pass
        ...
        try:
            idx_default = labels.index(current) if current in labels else 0
        except Exception:
            idx_default = 0
        choice = st.selectbox("Meny", labels, index=idx_default)
        if choice != current:
            _set_query_page(choice)
        return choice
...
# -----------------------------
def main():
    # Säkerställ nödvändiga globala helpers finns (definieras i tidigare delar)
    required_syms = [
        "read_data_df", "write_data_df", "get_settings_map", "get_fx_map",
        "DATA_COLUMNS", "SETTINGS_TITLE", "SETTINGS_COLUMNS", "SNAPSHOT_TITLE",
        "_f", "_pos", "_nz", "DEFAULT_BUCKETS", "fetch_from_yahoo"
    ]
    missing = [s for s in required_syms if s not in globals()]
    if missing:
        st.error(f"Saknade symboler i tidigare delar: {', '.join(missing)}")
        st.stop()

    # Ladda in data & kartor i session
    _load_all_into_session(force=False)

    # Rendera sidebar och välj sida
    page_label = _render_sidebar()
    pages = _router_pages()
    view = pages.get(page_label, {}).get("fn")
    ...
    try:
        if callable(view):
            ...
            st.error("Vald vy saknar render-funktion.")
    except Exception as e:
        st.error(f"💥 Fel i huvudloopen: {e}")

    # Footer / info
    st.markdown("---")
    st.caption("© Aktieanalys & investeringsförslag — Basversion (låst).")
