...
# --------------------------------------------
# Hjälpare för ranking (legacy-kompat, används ej längre direkt)
# --------------------------------------------

def _normalize_cmr_result(res) -> Tuple[Optional[pd.DataFrame], Dict[str, Any]]:
    """
    Normalisera retur från compute_methods för äldre varianter.
    Lämnas kvar för bakåtkompatibilitet men används inte i nya _rank_one.
    """
...
def _try_compute_methods(row: pd.Series) -> Tuple[Optional[pd.DataFrame], Dict[str, Any]]:
    """
    Legacy-hjälpare som försökte olika signaturer.
    Behålls för bakåtkompat, används inte längre av _rank_one.
    """
    try:
        res = compute_methods_for_row(row)  # type: ignore
...
def _extract_fair_values(methods_df: Optional[pd.DataFrame], meta: Dict[str, Any]) -> Dict[str, Optional[float]]:
    """
    Legacy-extraktion av fair value från meta/methods_df.
    Behålls endast som reserv, inte använd i nya flödet.
    """
...
# CHANGED: Ranking använder nu compute_methods_for_row v3-auto direkt
#          och hämtar riktkurser från payloadens target_* (fundamenta),
#          inte från en separat fair_value-extraktor.
def _rank_one(row: pd.Series, settings: Dict[str, str], fx_map: Dict[str, float]) -> Dict[str, Any]:
    tkr = str(row.get("Ticker") or "").upper().strip()
    ccy = str(_nz(row.get("Valuta"), "SEK")).upper()
    try:
        payload = compute_methods_for_row(row, settings, fx_map)
    except Exception as e:
        price_fallback = _f(row.get("Aktuell kurs"))
        return {
            "Ticker": tkr,
            "Valuta": ccy,
            "Kurs": _r2(price_fallback),
            "FV idag": None,
            "FV 1 år": None,
            "FV 2 år": None,
            "FV 3 år": None,
    ...
    price   = _f(payload.get("price") or row.get("Aktuell kurs"))
    fv_today = _f(payload.get("target_today"))
    fv_1y    = _f(payload.get("target_1y"))
    fv_2y    = _f(payload.get("target_2y"))
    fv_3y    = _f(payload.get("target_3y"))
    ...
    if _pos(price) and _pos(fv_today):
        upct = (float(fv_today) - float(price)) / float(price) * 100.0
    ...
    return {
        ...
        "Valuta": ccy,
        "Kurs": _r2(price),
        "FV idag": _r2(fv_today),
        "FV 1 år": _r2(fv_1y),
        "FV 2 år": _r2(fv_2y),
        "FV 3 år": _r2(fv_3y),
        "Bull 1 år": _r2(bull_1y),
        "Bear 1 år": _r2(bear_1y),
...
def page_ranking():
    ...
    with c3:
        st.caption("Beräkning enligt beräkningsmotor (Del 3) – riktkurser 1–3 år baseras på fundamenta (EPS/Rev + multiplar).")
    ...
    for i, t in enumerate(target, start=1):
        ...
        try:
            ...
            res = _rank_one(row, settings, fx_map)
            rows_out.append(res)
