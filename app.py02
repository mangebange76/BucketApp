...
# -------------------------
# Hjälpare: datumhantering  (HARDENED)
# -------------------------
def _parse_date_any(x: Any) -> Optional[dt.date]:
    """
    Försök tolka str/number/datetime/pandas.Timestamp till *datum* (utan tid).
    Tomt/ogiltigt/NaT → None.
    """
    import pandas as pd
    ...
    # Pandas Timestamp (inkl NaT)
    if isinstance(x, pd.Timestamp):
        if x is pd.NaT or pd.isna(x):
            return None
        return x.to_pydatetime().date()

    # Python datetime/datum
    if isinstance(x, dt.datetime):
        return x.date()
    if isinstance(x, dt.date):
        return x

    # Strängar / övrigt → försök parsa
    s = str(x).strip()
    if s == "" or s.lower() in ("nat", "nan", "none", "null"):
        return None

    # Vanliga format först
...
def _is_future_or_today(d: Any) -> bool:
    """
    True om datumet är idag eller framtid. Tål NaT/pandas.Timestamp/str.
    """
    import pandas as pd
