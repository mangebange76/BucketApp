...
#  • Init/Bootstrap (DATA + FX + Settings)
#  • Sidopanel (meny, snabbladdning, status)
#  • Routing till samtliga vyer
# ============================================================

def _bootstrap_once():
    """Läs in Settings, FX och DATA till session_state vid start."""
    ss = st.session_state
    # Settings
    try:
    ...
    try:
        fx_map = get_fx_map()
        if not fx_map:
            # Tomt – försök hämta och skriv
            _load_fx_and_update_sheet()
            fx_map = get_fx_map()
        ss["FX"] = fx_map or {}
    except Exception as e:
        st.warning(f"Kunde inte läsa/hämta Valutakurser: {e}")
        ss["FX"] = {}

    # DATA
    try:
        data_df = read_data_df()
        # Säkerställ kolumnschema om bladet är tomt/nytt
        if data_df is None or data_df.empty:
            data_df = pd.DataFrame(columns=DATA_COLUMNS)
            write_data_df(data_df[DATA_COLUMNS])
        # Garanti: alla kända kolumner finns
        for c in DATA_COLUMNS:
            if c not in data_df.columns:
                data_df[c] = np.nan
        ss["DATA"] = data_df
    except Exception as e:
        st.error(f"Kritiskt: kunde inte läsa Data-bladet: {e}")
        ss["DATA"] = pd.DataFrame(columns=DATA_COLUMNS)

    # Auto-refresh FX vid start?
    try:
        auto = str(ss["SETTINGS_MAP"].get("auto_refresh_on_start", "0")) == "1"
        if auto:
            _load_fx_and_update_sheet()
            ss["FX"] = get_fx_map()
    except Exception:
...
def _sidebar():
    ...
    default_page = ss.get("PAGE_CURRENT", "Analys")
    if default_page not in pages:
        default_page = "Analys"
    sel = st.sidebar.radio("Vyer", pages, index=pages.index(default_page), key="PAGE_CURRENT")

    st.sidebar.markdown("---")
    st.sidebar.markdown("### ⚡ Snabbåtgärder")
    col1, col2 = st.sidebar.columns(2)
    with col1:
        if st.button("↻ Ladda DATA", use_container_width=True):
            try:
                ss["DATA"] = read_data_df()
                st.sidebar.success("DATA uppdaterad.")
            except Exception as e:
                st.sidebar.error(f"DATA-fel: {e}")
    with col2:
        if st.button("↻ Ladda FX", use_container_width=True):
            try:
                _load_fx_and_update_sheet()
                ss["FX"] = get_fx_map()
                st.sidebar.success("FX uppdaterad.")
            except Exception as e:
                st.sidebar.error(f"FX-fel: {e}")

    st.sidebar.markdown("---")
    # Liten statusruta
    try:
        n = 0 if ss.get("DATA") is None else len(ss["DATA"])
        base_ccy = ss.get("SETTINGS_MAP", {}).get("primary_currency", "SEK")
        st.sidebar.caption(f"**Rader i DATA:** {n} · **Primär valuta:** {base_ccy}")
    except Exception:
        pass
...
def main():
    # Titel & layout satt i Del 1/6
    # Bootstrap (endast första gången)
    ...
    if page == "Settings":
        ...
        # Fallback till Analys
        page_analysis()

    # Footer
    st.markdown("---")
    st.caption("© Aktieanalys & investeringsförslag — Fair Value inkluderad (median över metoder).")
