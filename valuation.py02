...
#  - DCF-motor på FCF (auto-parametrar per Bucket)  <-- NYTT
#  - AUTO-PROFIL: väljer P/E vs EV (per sektor/mått)
#  - Metodpriser: PE, EV/S, EV/EBITDA, P/B (+ placeholders för struktur)
#  - Multipel-decay & PE-ankare
#  - Fair Value idag = DCF (primärt) med PE/EV-hybrid som fallback
...
# -------------------------
# DCF – auto-parametrar & PV från FCF  (NYTT)
# -------------------------

@st.cache_data(ttl=900, show_spinner=False)
def _fetch_fcf_ttm_yahoo(ticker: str) -> Optional[float]:
    """
    Hämtar senaste Free Cash Flow (TTM) från Yahoo via yfinance.
    Används endast för DCF – oberoende av yahoo_fetch_for_ticker().
    """
    try:
        tk = yf.Ticker(ticker)
        cf = tk.cashflow
        ...
        for row_name in ["Free Cash Flow", "FreeCashFlow", "Free CashFlow"]:
...
def _auto_dcf_params(
    bucket_label: Any,
    rev_cagr_hist: Optional[float],
    eps_cagr_long: Optional[float],
) -> Tuple[float, float, float]:
    """
    Sätter auto-parametrar till DCF utifrån:
      - Bucket (A/B/C, tillväxt/utdelning)
      - historisk/estimerad tillväxt
    Returnerar: (g_1_5, g_terminal, discount_rate)
    """

    # 1) Bas-tillväxt från EPS-longterm, annars Rev-CAGR, annars 0
    g_base = 0.0
    for cand in (eps_cagr_long, rev_cagr_hist, 0.0):
        if cand is not None:
            try:
                g_base = float(cand)
                break
    ...
    # Clamp – inte låta DCF spåra ur
    g_base = max(-0.05, min(0.25, g_base))  # -5%..+25%

    s = str(bucket_label or "").lower()

    # Bucket-justering (mer försiktig ju längre ned i bucket)
    adj = 0.0
    if "bucket a" in s:
        adj = 0.01      # lite plus
    elif "bucket b" in s:
        ...
        adj = -0.03     # rejäl försiktighet

    g_1_5 = g_base + adj
    g_1_5 = max(-0.05, min(0.25, g_1_5))

    # Terminaltillväxt – kopplat till “moat-nivå”
    if "bucket a" in s:
        g_terminal = 0.02
    elif "bucket b" in s:
        g_terminal = 0.0175
    ...
    # Diskonteringsränta – enkel riskmatris
    base_r = 0.085  # 8,5 % som bas
    is_growth = any(k in s for k in ["tillväxt", "tillvaxt", "growth"])

    if "bucket a" in s:
        r = base_r + (0.01 if is_growth else 0.005)   # ca 9–9,5%
    elif "bucket b" in s:
        r = base_r + (0.03 if is_growth else 0.02)    # ca 10,5–11,5%
    elif "bucket c" in s:
        r = base_r + (0.07 if is_growth else 0.05)    # ca 13,5–15,5%
    else:
        r = base_r + 0.02                             # default runt 10,5%

    return float(g_1_5), float(g_terminal), float(r)


def _dcf_equity_value_from_fcf(
    fcf_start: float,
    growth_1_5: float,
    g_terminal: float,
    discount_rate: float,
    ...
    """
    Klassisk DCF på Free Cash Flow to Firm:
      - FCF växer med growth_1_5 i 'years' år
      - Terminalvärde via Gordon Growth med g_terminal
      - Returnerar NUVÄRDET av framtida FCF (EV, före nettoskuld)
    """
    try:
        fcf = float(fcf_start)
    ...
    if not math.isfinite(fcf) or fcf <= 0:
        return None
    ...
    # Terminalvärde (Gordon) – skydd mot r <= g
    if r > gt:
        terminal = cf * (1.0 + gt) / (r - gt)
        total_pv += terminal / ((1.0 + r) ** years)

    return total_pv


# -------------------------
# AUTO-PROFIL: välj PE vs EV per bolag
# -------------------------
def _auto_method_profile(row: pd.Series, y_snap: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Idé:
      - Banker/finans/REIT/BDC → primärt P/B.
      - Tech/health/growth med orimligt P/E men vettig EV/S → primärt EV/S.
      - Övriga → primärt P/E om möjligt, annars EV/S/EV/EBITDA som fallback.
    """
    ...
    growth_like = is_tech or is_health
    ...
    if growth_like and allow["ev_s"]:
        if (pe_level is None) or (pe_level >= 50.0) or (not allow["pe"]):
            ...
            primary_family = "pe" if allow["pe"] else "ev_s"
    else:
        primary_family = None
...
# -------------------------
# Fair Value via familjemedian (v3 med filtrering + primary)
# -------------------------
def _compute_fair_value_row_v3(
    methods_df: pd.DataFrame,
    now_price: Optional[float],
    allow_fams: Dict[str, bool],
    primary_fam: Optional[str] = None,
) -> Dict[str, Any]:
    fam_map = {
        "pe_hist_vs_eps": "pe",
        "ev_sales": "ev_s",
    ...
    for c in cols:
        ...
        for _, r in methods_df.iterrows():
            ...
            if not allow_fams.get(fam, False):
                continue
            if primary_fam and fam != primary_fam:
                continue
            v = _f(r.get(c))
            if v is None:
                continue
...
# -------------------------
# Huvud: compute_methods_for_row → DICT (auto-profil + DCF)
# -------------------------
def compute_methods_for_row(
    ...
    # --- DCF-baserad FV (auto-parametrar) – PRIMÄR FV IDAG ---
    dcf_fv_today: Optional[float] = None
    dcf_params: Dict[str, Any] = {}
    try:
        bucket_label_dcf = str(_nz(row.get("Bucket"), "") or "")
        fcf_ttm = _fetch_fcf_ttm_yahoo(ticker)
        if fcf_ttm is not None and shares:
            g_1_5, g_term, r_dcf = _auto_dcf_params(
                bucket_label_dcf,
                rev_cagr_hist,
                eps_cagr_long,
            )
            ev_pv = _dcf_equity_value_from_fcf(
                fcf_start=float(fcf_ttm),
                growth_1_5=g_1_5,
                g_terminal=g_term,
                discount_rate=r_dcf,
            )
            if ev_pv is not None:
                eq_val = ev_pv - _nz(net_debt, 0.0)
                dcf_fv_today = eq_val / float(shares)
                dcf_params = {
                    "fcf_ttm": float(_pos(fcf_ttm)),
                    "g_1_5": g_1_5,
                    "g_terminal": g_term,
                    "discount_rate": r_dcf,
                }
    ...
    fv_row = _compute_fair_value_row_v3(methods_df, price, allow_fams, primary_fam)

    # Om PE-FV finns och primär familj är PE → använd PE-hybriden för "Idag"
    if fv_today_pe is not None and primary_fam == "pe":
        fv_row["Idag"] = fv_today_pe

    # --- DCF som PRIMÄR FV IDAG om tillgänglig ---
    if dcf_fv_today is not None:
        fv_row["Idag"] = dcf_fv_today

    # --- Bästa scenario (inom vald familj) ---
    ...
    target_today_dcf = _f(dcf_fv_today)
    ...
    payload: Dict[str, Any] = {
        ...
        "bull_1y": None,
        "bear_1y": None,
        "target_today_dcf": target_today_dcf,  # DCF-FV (kan skilja från fallback)
...
def compute_fair_values_for_row(
    ...
    fv_today_dcf = _f(payload.get("target_today_dcf"))

    # Nya nivåer runt FV idag (baserat på primär FV – nu DCF om finns)
